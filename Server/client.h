...
// Sound effect macros (should match client)
#define SFX_SPELL		1
#define SFX_DIE			2
#define SFX_DODGE		3
#define SFX_HIT			4
...
struct player
{
	...
	struct cplayer cpl;
	struct cmap cmap[RENDERDIST*RENDERDIST];
	struct cmap smap[RENDERDIST*RENDERDIST];
	
	// copy of map for comparision
	struct map xmap[RENDERDIST*RENDERDIST];
	
	// copy of visibility map for comparision
	int vx,vy;
	char visi[60*60]; // Remember to update to data.h's VISI_SIZE!!

	// instance IDs of last area transported to
	short inst_buffer[7];
	...
	int changed_field[RENDERDIST*RENDERDIST];	// IDs of changed fields
};
