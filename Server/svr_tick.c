...
void plr_cmd_move(int nr)
{
        ...
        if (ch[cn].flags&CF_TPMODE) {
                god_transfer_char(cn, x, y, ch[cn].instance_id);
        } else {
                ch[cn].goto_x=x;
                ch[cn].goto_y=y;
        }
...
void plr_cmd_turn(int nr)
{
        ...
        cmap=player[nr].cmap;
        smap=player[nr].smap;
        cmap_x = x - (ch[cn].x - RENDERDIST/2);
        cmap_y = y - (ch[cn].y - RENDERDIST/2);

        inst_id = ch[cn].instance_id;
        if (inst_id == -1) m = x + y * MAPX;
        else m = x + y * map_instances[inst_id].width;

        if (IS_BUILDING(cn)) {
                if (inst_id == -1) {
                        do_char_log(cn,3,"x=%d, y=%d, m=%d,light=%d, indoors=%lld, dlight=%d.\n",
                                x,y,m,map[m].light,map[m].flags&MF_INDOORS,map[m].dlight);

                        do_char_log(cn,3,"ch=%d, to_ch=%d, it=%d.\n",
                                map[m].ch,map[m].to_ch,map[m].it);
                        do_char_log(cn,3,"flags=%04X %04X\n",
                                (unsigned int) (map[m].flags>>32),
                                (unsigned int) (map[m].flags&0xFFFF));

                        do_char_log(cn,3,"sprite=%d, fsprite=%d\n",
                                map[m].sprite,map[m].fsprite);
                } else {
                        do_char_log(cn,3,"x=%d, y=%d, m=%d,light=%d, indoors=%lld, dlight=%d.\n",
                                x,y,m,map_instancedtiles[inst_id][m].light,map_instancedtiles[inst_id][m].flags&MF_INDOORS,map_instancedtiles[inst_id][m].dlight);

                        do_char_log(cn,3,"ch=%d, to_ch=%d, it=%d.\n",
                                map_instancedtiles[inst_id][m].ch,map_instancedtiles[inst_id][m].to_ch,map_instancedtiles[inst_id][m].it);
                        do_char_log(cn,3,"flags=%04X %04X\n",
                                (unsigned int) (map_instancedtiles[inst_id][m].flags>>32),
                                (unsigned int) (map_instancedtiles[inst_id][m].flags&0xFFFF));

                        do_char_log(cn,3,"sprite=%d, fsprite=%d\n",
                                map_instancedtiles[inst_id][m].sprite,map_instancedtiles[inst_id][m].fsprite);
                }
        } else {
                // Placing instance template characters
                if (ch[cn].data[91]) {
                        if (inst_id != -1) {
                                int ct = map_instancedtiles[inst_id][m].ch;
                                if (ct && ch[ct].flags&CF_INST_TEMP) {
                                        // Remove exising instance template on that spot
                                        do_char_log(cn, 1, "Removed instance template for %s at [%d;%d].\n", ch[ct].name, x, y);
                                        chlog(cn,"CREATOR: removed instance template char %s at [%d;%d].", ch[ct].name, x, y);
                                        god_destroy_items(ct);
                                        ch[ct].used = USE_EMPTY;
                                } else {
                                        if ((ct=god_createinstchar(ch[cn].data[91], x, y, inst_id))) {
                                                do_char_log(cn, 1, "Placed instance template for %d(%s) at [%d;%d].\n", ch[cn].data[91], ch[ct].name, x, y);
                                                chlog(cn,"CREATOR: created instance template char %d(%s) at [%d;%d].", ch[cn].data[91], ch[ct].name, x, y);
                                        } else {
                                                do_char_log(cn, 1, "Failed to place instance template at [%d;%d].\n", x, y);
                                        }
                                }
                        } else {
                                do_char_log(cn, 1, "No longer in instance. Instance template character mode toggled off.\n");
                                ch[cn].data[91] = 0;
                        }
                }
        }
...
void plr_cmd_drop(int nr)
{
        ...
        if (IS_BUILDING(cn)) {
                if (ch[cn].misc_action==DR_AREABUILD2) {
                        ...
                        for (x=xs; x<=xe; x++)
                                for (y=ys; y<=ye; y++)
                                        build_drop(x,y,ch[cn].citem,ch[cn].instance_id,0);
...
void plr_cmd_pickup(int nr)
{
        ...
        if (IS_BUILDING(cn)) {
                build_remove(x,y,ch[cn].instance_id);
                return;
        }
...
void plr_cmd_joininst(int nr)
{
        int inst_bufind, cn, mode;

        cn=player[nr].usnr;

        inst_bufind=*(unsigned int*)(player[nr].inbuf+1);

        do_joininst(cn, player[nr].inst_buffer[inst_bufind]);
...
void plr_cmd_look_item(int nr)
{
        ...
        if (inst_id == -1) {
                ...
                if (x<0 || x>=map_instances[inst_id].width || y<0 || y>=map_instances[inst_id].height) return;

                in=map_instancedtiles[inst_id][x+y*map_instances[inst_id].width].it;
        }
...
void plr_idle(int nr)
{
	if (globs->ticker-player[nr].lasttick>TICKS*60) {
        ...
        if (globs->ticker-player[nr].lasttick2>TICKS*60*PLR_IDLETIMER) {
...
void plr_newlogin(int nr)
{
	...
	if ((tmp=cap(0,nr))) {
		...
		*(unsigned int*)(buf+5)=player[nr].prio;
        ...
        if (!god_drop_char_fuzzy_large(cn,ch[cn].temple_x,ch[cn].temple_y,ch[cn].temple_x,ch[cn].temple_y,-1))
                if (!god_drop_char_fuzzy_large(cn,ch[cn].temple_x+3,ch[cn].temple_y,ch[cn].temple_x,ch[cn].temple_y,-1))
                        if (!god_drop_char_fuzzy_large(cn,ch[cn].temple_x,ch[cn].temple_y+3,ch[cn].temple_x,ch[cn].temple_y,-1)) {
...
void plr_login(int nr)
{
	...
	char_add_net(cn,ch[cn].addr);
        ...
        // Character was previously on an instanced area
        ...
        if (!(ch[cn].flags&CF_CCP)) {
                if (!god_drop_char_fuzzy_large(cn,ch[cn].tavern_x,ch[cn].tavern_y,ch[cn].tavern_x,ch[cn].tavern_y,-1))
                        if (!god_drop_char_fuzzy_large(cn,ch[cn].tavern_x+3,ch[cn].tavern_y,ch[cn].tavern_x,ch[cn].tavern_y,-1))
                                if (!god_drop_char_fuzzy_large(cn,ch[cn].tavern_x,ch[cn].tavern_y+3,ch[cn].tavern_x,ch[cn].tavern_y,-1)) {
...
void plr_logout(int cn,int nr,int reason)
{
        ...
        if (cn>0 && cn<MAXCHARS && (ch[cn].player==nr || nr==0) && (ch[cn].flags&(CF_PLAYER)) && !(ch[cn].flags&CF_CCP)) {
                ...
                if (reason==LO_EXIT) {
                        ...
                        if (inst_id == -1) do_area_log(cn,0,ch[cn].x,ch[cn].y,2,"%s left the game without saying goodbye and was punished by the gods.\n",ch[cn].name);
                        else do_area_log_inst(inst_id,cn,0,ch[cn].x,ch[cn].y,2,"%s left the game without saying goodbye and was punished by the gods.\n",ch[cn].name);
                }

                if (inst_id == -1) {
                        if (map[ch[cn].x+ch[cn].y*MAPX].ch==cn) {
                                map[ch[cn].x+ch[cn].y*MAPX].ch=0;
                                if (ch[cn].light) do_add_light(ch[cn].x,ch[cn].y,-ch[cn].light,inst_id);
                        }
                        if (map[ch[cn].tox+ch[cn].toy*MAPX].to_ch==cn) map[ch[cn].tox+ch[cn].toy*MAPX].to_ch=0;
                } else {
                        if (map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].ch==cn) {
                                map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].ch=0;
                                if (ch[cn].light) do_add_light(ch[cn].x,ch[cn].y,-ch[cn].light,inst_id);
                        }
                        if (map_instancedtiles[inst_id][ch[cn].tox + ch[cn].toy * map_instances[inst_id].width].to_ch==cn)
                                map_instancedtiles[inst_id][ch[cn].tox + ch[cn].toy * map_instances[inst_id].width].to_ch=0;
                }
                remove_enemy(cn);
                ...
                if (reason==LO_IDLE || reason==LO_SHUTDOWN || reason==0) { // give lag scroll to player
                        int nolag = 0;
                        if (inst_id == -1) {
                                if (map[ch[cn].x + ch[cn].y * MAPX].flags&MF_NOLAG) nolag = 1;
                        } else {
                                if (map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&MF_NOLAG) nolag = 1;
                        }
                        if (abs(ch[cn].x-ch[cn].temple_x)+abs(ch[cn].y-ch[cn].temple_y)>10 && !nolag) {
                                in=god_create_item(IT_LAGSCROLL);
                                it[in].data[0]=ch[cn].x;
                                it[in].data[1]=ch[cn].y;
                                it[in].data[2]=globs->ticker;

                                // Leads back to instance if still alive
                                if (inst_id != -1) {
                                        it[in].data[3] = 1;
...
int cl_light_21(int n,int dosend,struct cmap *cmap,struct cmap *smap)
{
	char buf[16],p;
	int m,l=0;

	if (!dosend) {
		for (m=n; m<n+27 && m<RENDERDIST*RENDERDIST; m++) {
		...
		for (m=n+2,p=6; m<min(n+20+2,RENDERDIST*RENDERDIST); m+=2,p++) {
...
int cl_light_three(int n,int dosend,struct cmap *cmap,struct cmap *smap)
{
	...
	if (!dosend) {
		for (m=n; m<n+3 && m<RENDERDIST*RENDERDIST; m++) {
			if (cmap[m].light!=smap[m].light) l++;
		}
		...
		for (m=n+2,p=6; m<min(n+2+2,RENDERDIST*RENDERDIST); m+=2,p++) {
			*(unsigned char*)(buf+p)=smap[m].light|(smap[m-1].light<<4);
			cmap[m].light=smap[m].light;
			cmap[m-1].light=smap[m-1].light;
		}
...
int cl_light_seven(int n,int dosend,struct cmap *cmap,struct cmap *smap)
{
	...
	if (!dosend) {
		for (m=n; m<n+7 && m<RENDERDIST*RENDERDIST; m++) {
			if (cmap[m].light!=smap[m].light) l++;
		}
		...
		for (m=n+2,p=6; m<min(n+6+2,RENDERDIST*RENDERDIST); m+=2,p++) {
			*(unsigned char*)(buf+p)=smap[m].light|(smap[m-1].light<<4);
			cmap[m].light=smap[m].light;
			cmap[m-1].light=smap[m-1].light;
		}
...
void plr_change(int nr)
{
	...
	if (cpl->a_end!=(ch[cn].a_end+500)/1000) {
        ...
        if (cpl->x!=ch[cn].x || cpl->y!=ch[cn].y) {
                if (cpl->x==ch[cn].x-1 && cpl->y==ch[cn].y) {
                        ...
                        memmove(cmap,cmap+1,sizeof(struct cmap)*(RENDERDIST*RENDERDIST-1));
                } else if (cpl->x==ch[cn].x+1 && cpl->y==ch[cn].y) {
                        buf[0]=SV_SCROLL_LEFT;
                        xsend(nr,buf,1);

                        memmove(cmap+1,cmap,sizeof(struct cmap)*(RENDERDIST*RENDERDIST-1));
                } else if (cpl->x==ch[cn].x && cpl->y==ch[cn].y-1) {
                        buf[0]=SV_SCROLL_DOWN;
                        xsend(nr,buf,1);

                        memmove(cmap,cmap+RENDERDIST,sizeof(struct cmap)*(RENDERDIST*RENDERDIST-RENDERDIST));
                } else if (cpl->x==ch[cn].x && cpl->y==ch[cn].y+1) {
                        buf[0]=SV_SCROLL_UP;
                        xsend(nr,buf,1);

                        memmove(cmap+RENDERDIST,cmap,sizeof(struct cmap)*(RENDERDIST*RENDERDIST-RENDERDIST));
                } else if (cpl->x==ch[cn].x+1 && cpl->y==ch[cn].y+1) {
                        buf[0]=SV_SCROLL_LEFTUP;
                        xsend(nr,buf,1);

                        memmove(cmap+RENDERDIST+1,cmap,sizeof(struct cmap)*(RENDERDIST*RENDERDIST-RENDERDIST-1));
                } else if (cpl->x==ch[cn].x+1 && cpl->y==ch[cn].y-1) {
                        buf[0]=SV_SCROLL_LEFTDOWN;
                        xsend(nr,buf,1);

                        memmove(cmap,cmap+RENDERDIST-1,sizeof(struct cmap)*(RENDERDIST*RENDERDIST-RENDERDIST+1));
                } else if (cpl->x==ch[cn].x-1 && cpl->y==ch[cn].y+1) {
		...
		buf[0]=SV_SETORIGIN;
		*(short*)(buf+1)=ch[cn].x-RENDERDIST/2; //smap[0].x;
		*(short*)(buf+3)=ch[cn].y-RENDERDIST/2; //smap[0].y;
		xsend(nr,buf,5);
        }
        ...
        for (n=0; n<RENDERDIST*RENDERDIST; n++) {
                if (cmap[n].light!=smap[n].light) {
                        ...
                        static int (*lfunc[])(int,int,struct cmap*,struct cmap*)=
                                {cl_light_one,cl_light_three,cl_light_seven,cl_light_21};
	...
	while (1) {
		...
		tmp=fdiff(cmap+n,smap+n,sizeof(struct cmap)*(RENDERDIST*RENDERDIST-n));	// find address of first difference
	...
	//for (f=0; (n=player[nr].changed_field[f])!=-1; f++) {	
		...
		if (n>=RENDERDIST*RENDERDIST) break;	// finished?
        }
...
inline int check_dlight(int x,int y,int inst_id)
{
        int m;
        ...
        if (inst_id == -1) {
                ...
                if (!(map_instancedtiles[inst_id][m].flags&MF_INDOORS)) return globs->dlight;

                return (globs->dlight*map[m].dlight)/256;
        }
}

int check_dlightm(int m,int inst_id)
{
        if (inst_id == -1) {
                if (!(map[m].flags&MF_INDOORS)) return globs->dlight;
                ...
                return (globs->dlight*map_instancedtiles[inst_id][m].dlight)/256;
        }
}

void empty_field(struct cmap *smap,int n)
{
...
void plr_getmap_complete(int nr)
{
        ...
        ys=ch[cn].y-(RENDERDIST/2)+YSCUT;
        ye=ch[cn].y+(RENDERDIST/2)-YECUT;
        xs=ch[cn].x-(RENDERDIST/2)+XSCUT;
        xe=ch[cn].x+(RENDERDIST/2)-XECUT;

        // trigger recomputation of visibility map
        can_see(cn,ch[cn].x,ch[cn].y,ch[cn].x+1,ch[cn].y+1,RENDERDIST/2,inst_id);
	...
	if (IS_BUILDING(cn)) do_all=1;
        ...
        for (n=YSCUT*RENDERDIST+XSCUT,m=xs+ys*m_wid,y=ys; y<ye; y++,m+=m_wid-RENDERDIST+XSCUT+XECUT,n+=XSCUT+XECUT) {
			...
			if (x<0 || y<0 || x>=m_wid || y>=m_hei) { empty_field(smap,n); continue; }

			// calculate light
			tmp=check_dlightm(m,inst_id);

			if (inst_id == -1) light=max(map[m].light,tmp);
                        else light=max(map_instancedtiles[inst_id][m].light,tmp);
			...
			if (IS_BUILDING(cn)) {
                                if (inst_id == -1) smap[n].flags2=(unsigned int)map[m].flags;
                                else smap[n].flags2=(unsigned int)map_instancedtiles[inst_id][m].flags;
                        } else smap[n].flags2=0;
			...
			// --- Begin of ba_sprite  -----
			if (inst_id == -1) smap[n].ba_sprite=map[m].sprite;
                        else smap[n].ba_sprite=map_instancedtiles[inst_id][m].sprite;
			// --- End of ba_sprite -----

			// --- Begin of Character ----- - also sets flags
                        if (inst_id == -1) co = map[m].ch;
                        else co = map_instancedtiles[inst_id][m].ch;
			if (visible && co != 0 && (tmp=do_char_can_see(cn,co))!=0) {
			...
			// --- Begin of Item -----
                        if (inst_id == -1) {
                                if (map[m].fsprite) {
                                        ...
                                        if ((co=map[m].it)!=0 && (!(it[co].flags&(IF_TAKE|IF_HIDDEN)) || (visible && do_char_can_see_item(cn,co)))) {
                                                if (it[co].active) {
                                ...
                                if (map_instancedtiles[inst_id][m].fsprite) {
                                        smap[n].it_sprite=map_instancedtiles[inst_id][m].fsprite;
                                        smap[n].it_status=0;
                                } else {
                                        if ((co=map_instancedtiles[inst_id][m].it)!=0 && (!(it[co].flags&(IF_TAKE|IF_HIDDEN)) || (visible && do_char_can_see_item(cn,co)))) {
                                                if (it[co].active) {
                                                        smap[n].it_sprite=it[co].sprite[1];
                                                        smap[n].it_status=it[co].status[1];
                                                } else {
...
void plr_getmap_fast(int nr)
{
        ...
        ys=ch[cn].y-(RENDERDIST/2)+YSCUTF;
        ye=ch[cn].y+(RENDERDIST/2)-YECUTF;
        xs=ch[cn].x-(RENDERDIST/2)+XSCUTF;
        xe=ch[cn].x+(RENDERDIST/2)-XECUTF;
	...
	if (IS_BUILDING(cn)) do_all=1;
        ...
        for (n=YSCUTF*RENDERDIST+XSCUTF,m=xs+ys*m_wid,y=ys; y<ye; y++,m+=m_wid-RENDERDIST+XSCUTF+XECUTF,n+=XSCUTF+XECUTF) {
                for (x=xs; x<xe; x++,n++,m++) {
                        if (inst_id == -1) {
                                if (do_all || map[m].it || map[m].ch || mcmp(&player[nr].xmap[n],&map[m],sizeof(struct map))) {	// any change on map data?
                                        mcpy(&player[nr].xmap[n],&map[m],sizeof(struct map)); 		// remember changed values
                                ...
                                if (do_all || map_instancedtiles[inst_id][m].it || map_instancedtiles[inst_id][m].ch || mcmp(&player[nr].xmap[n],&map_instancedtiles[inst_id][m],sizeof(struct map))) {	// any change on map data?
                                        mcpy(&player[nr].xmap[n],&map_instancedtiles[inst_id][m],sizeof(struct map)); 		// remember changed values
                                } else continue;
                        }
			...
			// item
                        if (inst_id == -1) {
                                ...
                                if (map_instancedtiles[inst_id][m].fsprite) {
                                        smap[n].it_sprite=map_instancedtiles[inst_id][m].fsprite;
                                        smap[n].it_status=0;
                                } else if ((co=map_instancedtiles[inst_id][m].it)!=0 && (!(it[co].flags&(IF_TAKE|IF_HIDDEN)) || (visible && do_char_can_see_item(cn,co)))) {
                                        if (it[co].active) {
                                                smap[n].it_sprite=it[co].sprite[1];
                                                smap[n].it_status=it[co].status[1];
                                        } else {
...
int check_valid(int cn)
{
        ...
        if (ch[cn].x<1 || ch[cn].y<1 || ch[cn].x>m_wid-2 || ch[cn].y>m_hei-2) {
                chlog(cn,"Killed character %s (%d) for invalid data",ch[cn].name,cn);
                do_char_killed(0,cn);
                return 0;
        }

        n=ch[cn].x+ch[cn].y*m_wid;

        if (inst_id == -1) {
                if (map[n].ch!=cn) {
                        chlog(cn,"Not on map (%d)!",map[n].ch);
                        if (map[n].ch) { chlog(cn,"drop=%d",god_drop_char_fuzzy_large(cn,900,900,900,900,inst_id)); }
                        else map[n].ch=cn;
                }
        } else {
                if (map_instancedtiles[inst_id][n].ch!=cn) {
                        chlog(cn,"Not on map (%d:%d)!",inst_id,map_instancedtiles[inst_id][n].ch);
                        if (map_instancedtiles[inst_id][n].ch) { chlog(cn,"drop=%d",god_drop_char_fuzzy_large(cn,900,900,900,900,inst_id)); }
                        else map_instancedtiles[inst_id][n].ch=cn;
                }
        }
...
{
        if (ch[cn].flags&CF_INST_TEMP) return 0; // Reaction-less instance templates
	...
	globs->players_online=plon;
        ...
        if (cltick>17) {
			...
			globs->load_avg=(int)((globs->load_avg*0.0099+globs->load*0.01)*100);
                ...
                // Update instances activity time
                for (n=1; n<MAXPLAYER; n++) {
                        if (!player[n].sock) continue;
                        if (player[n].state!=ST_NORMAL) continue;
                        
                        int inst_id = ch[player[n].usnr].instance_id;
                        if (inst_isalive(inst_id)) {
                                map_instances[inst_id].last_activity_time = globs->ticker;
                        }
                }

                // Instance expiration
                for (n=0; n<INST_MAX; n++) {
                        if (map_instances[n].used == USE_EMPTY) continue;
                        if (globs->ticker - map_instances[n].last_activity_time >= INST_DURATION) {
                                xlog("Instance %d (%s) expired, unloading...", n, map_instances[n].name);
                                unload_instance(n);
                        }
                }
        }
