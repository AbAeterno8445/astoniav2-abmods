...
int pathfinder(int cn,int tx,int ty,int flag,int x2,int y2,int inst_id);
...
int char_moveto(int cn,int x,int y,int flag,int x2,int y2)
{
	...
	if (ch[cn].cerrno==ERR_FAILED) return -1;

	if (ch[cn].unreach>globs->ticker && ch[cn].unreachx==x && ch[cn].unreachy==y) return -1;

	inst_id = ch[cn].instance_id;

	prof=prof_start();
	dir=pathfinder(cn,x,y,flag,x2,y2,inst_id);
	prof_stop(1,prof);
	...
	switch(dir) {
		case DX_RIGHTDOWN:
			if (ch[cn].dir!=DX_RIGHTDOWN) {
				if (TURNING_ENABLED==1) act_turn_rightdown(cn);
				else plr_turn_rightdown(cn);
				return 0;
			}
		...
		case DX_RIGHTUP:
			if (ch[cn].dir!=DX_RIGHTUP) {
				if (TURNING_ENABLED==1) act_turn_rightup(cn);
				else plr_turn_rightup(cn);
				return 0;
			}
			act_move_rightup(cn);
			return 0;
		case DX_LEFTDOWN:
			if (ch[cn].dir!=DX_LEFTDOWN) {
				if (TURNING_ENABLED==1) act_turn_leftdown(cn);
				else plr_turn_leftdown(cn);
				return 0;
			}
		...
		case DX_LEFTUP:
			if (ch[cn].dir!=DX_LEFTUP) {
				if (TURNING_ENABLED==1) act_turn_leftup(cn);
				else plr_turn_leftup(cn);
				return 0;
			}
			act_move_leftup(cn);
			return 0;
		case DX_RIGHT:
			if (ch[cn].dir!=DX_RIGHT) {
				if (TURNING_ENABLED==1) act_turn_right(cn);
				else plr_turn_right(cn);
				return 0;
			}
			if (inst_id == -1) {
				...
				if ((in=map_instancedtiles[inst_id][(ch[cn].x+1)+(ch[cn].y)*map_instances[inst_id].width].it)!=0 && !it[in].active && it[in].driver==2)
					{ act_use(cn); return 0; }
			}
			act_move_right(cn);
			return 0;
		case DX_LEFT:
			if (ch[cn].dir!=DX_LEFT) {
				if (TURNING_ENABLED==1) act_turn_left(cn);
				else plr_turn_left(cn);
				return 0;
			}
			if (inst_id == -1) {
				if ((in=map[(ch[cn].x-1)+(ch[cn].y)*MAPX].it)!=0 && !it[in].active && it[in].driver==2)
		...
		case DX_DOWN:
			if (ch[cn].dir!=DX_DOWN) {
				if (TURNING_ENABLED==1) act_turn_down(cn);
				else plr_turn_down(cn);
				return 0;
			}
			if (inst_id == -1) {
				...
				if ((in=map_instancedtiles[inst_id][(ch[cn].x)+(ch[cn].y+1)*map_instances[inst_id].width].it)!=0 && !it[in].active && it[in].driver==2)
					{ act_use(cn); return 0; }
			}
			act_move_down(cn);
			return 0;
		case DX_UP:
			if (ch[cn].dir!=DX_UP) {
				if (TURNING_ENABLED==1) act_turn_up(cn);
				else plr_turn_up(cn);
				return 0;
			}
			if (inst_id == -1) {
