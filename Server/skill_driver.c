...
int add_spell(int cn,int in)
{
        ...
        if (inst_id == -1) {
                if (map[ch[cn].x + ch[cn].y * MAPX].flags&CF_NOMAGIC) return 0;
        } else {
                if (map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&CF_NOMAGIC) return 0;
        }
        ...
        if (ch[cn].flags&CF_PLAYER && it[in].temp == SK_RECALL) {
                do_char_log(cn, 1, "Strange winds begin to surround you.\n");
        }
...
int spell_light(int cn,int co,int power)
{
	...
	power=spell_race_mod(power,ch[cn].kindred);
        ...
        if (cn!=co) {
                ...
                if (inst_id == -1) do_area_log(co,0,ch[co].x,ch[co].y,1,"%s starts to emit light.\n",ch[co].name);
                else do_area_log_inst(inst_id,co,0,ch[co].x,ch[co].y,1,"%s starts to emit light.\n",ch[co].name);

                char_play_sound(co,ch[cn].sound+1,-150,0);
                char_play_sound(cn,ch[cn].sound+1,-150,0);
                chlog(cn,"Cast Light on %s",ch[co].name);
                fx_add_effect(6,0,ch[co].x,ch[co].y,0,inst_id);
        } else {
                if (!add_spell(cn,in))  {
                        do_char_log(cn,1,"Magical interference neutralised the %s's effect.\n",it[in].name);
                        return 0;
...
int spell_curse(int cn,int co,int power)
{
	...
	power=spell_race_mod(power,ch[cn].kindred);
        ...
        fx_add_effect(5,0,ch[co].x,ch[co].y,0,inst_id);
...
void skill_warcry(int cn)
{
	...
	for (x=xf; x<xt; x++) {
		for (y=yf; y<yt; y++) {
                        if (inst_id == -1) co = map[x+y*MAPX].ch;
                        else co = map_instancedtiles[inst_id][x+y*map_instances[inst_id].width].ch;
			if (co) {
...
void skill_blast(int cn)
{
        ...
        m=ch[cn].x+ch[cn].y*m_wid;
        for (int i=0; i<4; i++) {
                int j = m;
                switch(i) {
                        case 0: j += 1; break;
                        case 1: j -= 1; break;
                        case 2: j += m_wid; break;
                        case 3: j -= m_wid; break;
                }
                if (inst_id == -1) co = map[j].ch;
                else co = map_instancedtiles[inst_id][j].ch;
                if (co!=0 && ch[co].attack_cn==cn && co_orig!=co) {
                        chlog(cn,"Cast Blast on %s",ch[co].name);
...
void skill_ghost(int cn)
{
        ...
        fx_add_effect(6,0,ch[cc].x,ch[cc].y,0,inst_id);
        fx_add_effect(7,0,ch[cn].x,ch[cn].y,0,inst_id);
