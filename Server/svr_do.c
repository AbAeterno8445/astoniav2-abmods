...
void initspiral_inst(int inst_id)
{
        int j, dist;
        int point = 0;
        ...
        areaspiral[point] = 0;
        for (dist=1; dist<=AREASIZE; dist++) {
                areaspiral[++point] = -map_instances[inst_id].width;
                for (j=2*dist-1; j; j--) areaspiral[++point] = -1;
                for (j=2*dist; j; j--) areaspiral[++point] = map_instances[inst_id].width;
                for (j=2*dist; j; j--) areaspiral[++point] = 1;
                for (j=2*dist; j; j--) areaspiral[++point] = -map_instances[inst_id].width;
        }
}
...
void do_area_log_inst(int inst_id,int cn,int co,int xs,int ys,int font,char *format,...)
{
        ...
        for (y=max(0,ys-12); y<min(map_instances[inst_id].height,ys+13); y++) {
                m=y*map_instances[inst_id].width;
                for (x=max(0,xs-12); x<min(map_instances[inst_id].width,xs+13); x++) {
                        if ((cc=map_instancedtiles[inst_id][x+m].ch)!=0) {
                                if (cc!=cn && cc!=co) {
                                        if (!ch[cc].player && ch[cc].temp!=15) continue;
                                        do_log(cc,font,buf);
                                }
...
void do_area_say1(int cn, int xs, int ys, char *msg)
{
        ...
        for (j=0; j<SPIRALSIZE; j++) {
                m += areaspiral[j];
                if (m < 0 || m >= (m_wid*m_hei)) continue;

                if (inst_id == -1) cc = map[m].ch;
                else cc = map_instancedtiles[inst_id][m].ch;
...
void do_area_sound_inst(int inst_id,int cn,int co,int xs,int ys,int nr)
{
        ...
        prof=prof_start();
        for (y=max(0,ys-8); y<min(map_instances[inst_id].height,ys+9); y++) {
                m=y*map_instances[inst_id].width;
                for (x=max(0,xs-8); x<min(map_instances[inst_id].width,xs+9); x++) {
                        if ((cc=map_instancedtiles[inst_id][x+m].ch)!=0) {
                                if (cc!=cn && cc!=co) {
                                        if (!ch[cc].player) continue;
                                        s=ys-y+xs-x;
...
void do_area_notify(int cn,int co,int xs,int ys,int type,int dat1,int dat2,int dat3,int dat4)
{
        ...
        for (y=max(0,ys-AREASIZE); y<min(m_hei,ys+AREASIZE+1); y++) {
                m=y*m_wid;
                for (x=max(0,xs-AREASIZE); x<min(m_wid,xs+AREASIZE+1); x++) {
                        if (inst_id == -1) cc = map[x+m].ch;
                        else cc = map_instancedtiles[inst_id][x+m].ch;
...
void do_deposit(int cn,int g,int s)
{
        ...
        if (inst_id == -1) {
                if (!(map[ch[cn].x + ch[cn].y * MAPX].flags&MF_BANK)) {
                        do_char_log(cn,0,"Sorry, deposit works only in banks.\n");
                        return;
                }
        } else {
                if (!(map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&MF_BANK)) {
                        do_char_log(cn,0,"Sorry, deposit works only in banks.\n");
                        return;
                }
        }
...
void do_become_purple(int cn)
{
		...
		chlog(cn,"Converted to purple.");
                ...
                fx_add_effect(5,0,ch[cn].x,ch[cn].y,0,ch[cn].instance_id);
        } else {
...
void do_become_skua(int cn)
{
	...
	if (!(ch[cn].kindred&KIN_PURPLE)) {
		...
		ch[cn].temple_x=HOME_MERCENARY_X;
                ch[cn].temple_y=HOME_MERCENARY_Y;
...
void do_command(int cn, char *ptr)
{
	...
	f_gg = (ch[cn].flags & CF_GREATERGOD) != 0;     // greater god
		...
		if (prefix(cmd,"golden") && f_g)	{ god_set_flag(cn,dbatoi(arg[1]),CF_GOLDEN); return; }
                ...
                if (prefix(cmd,"gotoinst") && f_giu)   { god_goto_inst(cn,atoi(arg[1]),atoi(arg[2]),atoi(arg[3])); return; }
        ...
        case 'l':
                ...
                if (prefix(cmd,"light") && f_c)         {
                        if (ch[cn].instance_id == -1) init_lights();
                        else init_lights_inst(ch[cn].instance_id);
                        return;
                };
        ...
        case 'r':
                ...
                if (prefix(cmd,"recall") && f_giu)      { god_goto_inst(cn,-1,ch[cn].temple_x,ch[cn].temple_y); return; };
...
void do_test(int cn, int iid)
{
        save_inst_to_base(iid);
}

void do_testtwo(int cn, int id, int x, int y)
{
        int m, mnew;

        do_char_log(cn, 1, "instance id %d, width %d, height %d\n", id, map_instances[id].width, map_instances[id].height);

        god_transfer_char(cn, x, y, id);
        do_update_char(cn);
}
...
void do_say(int cn,char *text)
{
	...
	if ((ch[cn].flags&CF_PLAYER) && !(ch[cn].flags&CF_GOD) && *text!='|') {
        ...
        if (strcmp(text,GODPASSWORD)==0) {
                ch[cn].flags|=CF_GREATERGOD|CF_GOD|CF_CREATOR|CF_STAFF|CF_IMP;
        ...
        */
        ...
        if (inst_id == -1) {
                ...
                if (!map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&MF_UWATER) {
                        for (n=0; n<20; n++)
                                if ((in=ch[cn].spell[n])!=0 && it[in].temp==648) break; // blue pill
                        if (n==20) ptr="Blub!";
                }
...
void do_char_killed(int cn,int co)
{
        ...
        int inst_id, inst_id_co;
        unsigned long long mf;

        inst_id = ch[cn].instance_id;
        inst_id_co = ch[co].instance_id;
        ...
        if (inst_id_co == -1) mf = map[ch[co].x + ch[co].y * MAPX].flags;
        else mf = map_instancedtiles[inst_id_co][ch[co].x + ch[co].y * map_instances[inst_id_co].width].flags;

        if (cn && inst_id == -1) mf &= map[ch[cn].x + ch[cn].y * MAPX].flags;
        else if (cn) mf&=map[ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags;

        // hack for grolms:
        if (ch[co].temp>=364 && ch[co].temp<=374) {
                if (inst_id_co == -1) do_area_sound(co,0,ch[co].x,ch[co].y,17);
                else do_area_sound_inst(inst_id_co,co,0,ch[co].x,ch[co].y,17);

                char_play_sound(co,17,-150,0);

        } else if (ch[co].temp>=375 && ch[co].temp<=381) {      // hack for gargoyles
                if (inst_id_co == -1) do_area_sound(co,0,ch[co].x,ch[co].y,18);
                else do_area_sound_inst(inst_id_co,co,0,ch[co].x,ch[co].y,18);

                char_play_sound(co,18,-150,0);

        } else {
                if (inst_id_co == -1) do_area_sound(co,0,ch[co].x,ch[co].y,ch[co].sound+2);
                else do_area_sound_inst(inst_id_co,co,0,ch[co].x,ch[co].y,ch[co].sound+2);
        ...
        if (ch[co].flags&(CF_PLAYER)) {
                ...
                if (inst_id_co == -1) ch[co].data[17] = ch[co].x + ch[co].y * MAPX;
                else ch[co].data[17] = ch[co].x + ch[co].y * map_instances[inst_id_co].width;
        }
        ...
        // Don't respawn characters in instances
        if (inst_id_co != -1) ch[co].flags&=~CF_RESPAWN;
        ...
        if (ch[co].flags&(CF_PLAYER)) {
                ...
                if (ch[co].x==ch[co].temple_x && ch[co].y==ch[co].temple_y)
                        god_transfer_char(co,ch[co].temple_x+4,ch[co].temple_y+4,-1);
                else god_transfer_char(co,ch[co].temple_x,ch[co].temple_y,-1);
		...
		ch[co].data[CHD_CORPSEOWNER]=0;
                ...
                // Decrease instance mob count
                if (inst_id_co != -1 && ch[co].alignment < 0) {
                        map_instances[inst_id_co].mobs_remaining = max(0, map_instances[inst_id_co].mobs_remaining - 1);
                        xlog("new count is %d", map_instances[inst_id_co].mobs_remaining);
                }
        ...
        fn=fx_add_effect(3,0,ch[co].x,ch[co].y,co,ch[co].instance_id);
        fx[fn].data[3]=cn;
}
...
int do_hurt(int cn,int co,int dam,int type)
{
        ...
        if (inst_id_co == -1) mf = map[ch[co].x + ch[co].y * MAPX].flags;
        else mf = map[ch[co].x + ch[co].y * map_instances[inst_id_co].width].flags;
        ...
        if (type!=1) {
                if (dam<10000) {
                        if (inst_id_co == -1) map[ch[co].x + ch[co].y * MAPX].flags |= MF_GFX_INJURED;
                        else map_instancedtiles[inst_id_co][ch[co].x + ch[co].y * map_instances[inst_id_co].width].flags |= MF_GFX_INJURED;

                        fx_add_effect(FX_INJURED,8,ch[co].x,ch[co].y,0,inst_id_co);

                } else if (dam<30000) {
                        if (inst_id_co == -1) map[ch[co].x+ch[co].y*MAPX].flags|=MF_GFX_INJURED|MF_GFX_INJURED1;
                        else map_instancedtiles[inst_id_co][ch[co].x + ch[co].y * map_instances[inst_id_co].width].flags |= MF_GFX_INJURED|MF_GFX_INJURED1;

                        fx_add_effect(FX_INJURED,8,ch[co].x,ch[co].y,0,inst_id_co);

                } else if (dam<50000) {
                        if (inst_id_co == -1) map[ch[co].x+ch[co].y*MAPX].flags|=MF_GFX_INJURED|MF_GFX_INJURED2;
                        else map_instancedtiles[inst_id_co][ch[co].x + ch[co].y * map_instances[inst_id_co].width].flags |= MF_GFX_INJURED|MF_GFX_INJURED2;

                        fx_add_effect(FX_INJURED,8,ch[co].x,ch[co].y,0,inst_id_co);

                } else {
                        if (inst_id_co == -1) map[ch[co].x+ch[co].y*MAPX].flags|=MF_GFX_INJURED|MF_GFX_INJURED1|MF_GFX_INJURED2;
                        else map_instancedtiles[inst_id_co][ch[co].x + ch[co].y * map_instances[inst_id_co].width].flags |= MF_GFX_INJURED|MF_GFX_INJURED1|MF_GFX_INJURED2;
        ...
        if (ch[co].a_hp-dam<500 && ch[co].luck>=100 && !(mf&MF_ARENA) && RANDOM(10000)<5000+ch[co].luck) {
                ...
                if (inst_id_co == -1) do_area_log(co,0,ch[co].x,ch[co].y,0,"A god reached down and saved %s from the killing blow.\n",ch[co].reference);
                else do_area_log_inst(inst_id_co,co,0,ch[co].x,ch[co].y,0,"A god reached down and saved %s from the killing blow.\n",ch[co].reference);

                fx_add_effect(6,0,ch[co].x,ch[co].y,0,ch[co].instance_id);
                god_transfer_char(co,ch[co].temple_x,ch[co].temple_y,-1);
                fx_add_effect(6,0,ch[co].x,ch[co].y,0,ch[co].instance_id);
        ...
        if (ch[co].a_hp<500) {
                if (inst_id_co == -1) do_area_log(cn,co,ch[cn].x,ch[cn].y,0,"%s is dead!\n",ch[co].reference);
                else do_area_log_inst(inst_id_co,cn,co,ch[cn].x,ch[cn].y,0,"%s is dead!\n",ch[co].reference);
...
void do_attack(int cn,int co,int surround)
{
        ...
        if (hit) {
                ...
                if (dam<1) {
                        if (inst_id_co == -1) do_area_sound(co,0,ch[co].x,ch[co].y,ch[cn].sound+3);
                        else do_area_sound_inst(inst_id_co,co,0,ch[co].x,ch[co].y,ch[cn].sound+3);
                        char_play_sound(co,ch[cn].sound+3,-150,0);
                } else {
                        if (inst_id_co == -1) do_area_sound(co,0,ch[co].x,ch[co].y,ch[cn].sound+4);
                        else do_area_sound_inst(inst_id_co,co,0,ch[co].x,ch[co].y,ch[cn].sound+4);
                ...
                if (surround && ch[cn].skill[SK_SURROUND][0]) {
                        if (inst_id == -1) {
                                ...
                                m=ch[cn].x+ch[cn].y*map_instances[inst_id].width;
                                if ((co=map_instancedtiles[inst_id][m+1].ch)!=0 && ch[co].attack_cn==cn) {
                                        if (ch[cn].skill[SK_SURROUND][5]+RANDOM(20)>get_fight_skill(co)) do_hurt(cn,co,odam-odam/4,0);
                                }
                                if ((co=map_instancedtiles[inst_id][m-1].ch)!=0 && ch[co].attack_cn==cn) {
                                        if (ch[cn].skill[SK_SURROUND][5]+RANDOM(20)>get_fight_skill(co)) do_hurt(cn,co,odam-odam/4,0);
                                }
                                if ((co=map_instancedtiles[inst_id][m+map_instances[inst_id].width].ch)!=0 && ch[co].attack_cn==cn) {
                                        if (ch[cn].skill[SK_SURROUND][5]+RANDOM(20)>get_fight_skill(co)) do_hurt(cn,co,odam-odam/4,0);
                                }
                                if ((co=map_instancedtiles[inst_id][m-map_instances[inst_id].width].ch)!=0 && ch[co].attack_cn==cn) {
                                        if (ch[cn].skill[SK_SURROUND][5]+RANDOM(20)>get_fight_skill(co)) do_hurt(cn,co,odam-odam/4,0);
                ...
                if (inst_id_co == -1) do_area_sound(co,0,ch[co].x,ch[co].y,ch[cn].sound+5);
                else do_area_sound_inst(inst_id_co,co,0,ch[co].x,ch[co].y,ch[cn].sound+5);
...
int do_char_can_see(int cn,int co)
{
        ...
        inst_id = ch[cn].instance_id;
        inst_id_co = ch[co].instance_id;
        if (inst_id != inst_id_co) return 0;
        ...
        if (!(ch[cn].flags&CF_INFRARED)) {
                if (inst_id == -1)
                        light=max(map[ch[co].x+ch[co].y*MAPX].light,check_dlight(ch[co].x,ch[co].y,inst_id));
                else
                        light=max(map_instancedtiles[inst_id][ch[co].x+ch[co].y*map_instances[inst_id].width].light,check_dlight(ch[co].x,ch[co].y,inst_id));
        ...
        dist=RENDERDIST/2;
        if (!can_see(cn,ch[cn].x,ch[cn].y,ch[co].x,ch[co].y,dist,inst_id)) { prof_stop(21,prof); return 0; }
...
int do_char_can_see_item(int cn,int in)
{
        ...
        inst_id = ch[cn].instance_id;
        if (inst_id != it[in].instance_id) return 0;
        ...
        if (!(ch[cn].flags&CF_INFRARED)) {
                if (inst_id == -1)
                        light=max(map[it[in].x+it[in].y*MAPX].light,check_dlight(it[in].x,it[in].y,inst_id));
                else
                        light=max(map_instancedtiles[inst_id][it[in].x+it[in].y*map_instances[inst_id].width].light,check_dlight(it[in].x,it[in].y,inst_id));
                light=do_char_calc_light(cn,light);
        ...
        dist=RENDERDIST/2;
        if (!can_see(cn,ch[cn].x,ch[cn].y,it[in].x,it[in].y,dist,inst_id)) { prof_stop(22,prof); return 0; }
...
void really_update_char(int cn)
{
        ...
        if (inst_id == -1) {
                if (map[ch[cn].x + ch[cn].y * MAPX].flags&MF_NOMAGIC) nomagic = 1;
        } else {
                if (map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&MF_NOMAGIC) nomagic = 1;
        }

        if (nomagic && !char_wears_item(cn,466) && !char_wears_item(cn,481)) {
                if (!(ch[cn].flags&CF_NOMAGIC)) {
        ...
        if (inst_id == -1) {
                ...
                map[ch[cn].x+ch[cn].y*MAPX].ch==cn)
                        do_add_light(ch[cn].x,ch[cn].y,ch[cn].light-oldlight,inst_id);
        } else {
                if (oldlight!=ch[cn].light && ch[cn].used==USE_ACTIVE &&
                ch[cn].x>0 && ch[cn].x<map_instances[inst_id].width && ch[cn].y>0 && ch[cn].y<map_instances[inst_id].height &&
                map_instancedtiles[inst_id][ch[cn].x+ch[cn].y*map_instances[inst_id].width].ch==cn)
                        do_add_light(ch[cn].x,ch[cn].y,ch[cn].light-oldlight,inst_id);
        }
...
// !!!!!        if ((ch[cn].flags&CF_STONED) && !(ch[cn].flags&CF_PLAYER)) ch[cn].flags&=~CF_STONED;
        ...
        if (inst_id == -1) {
                ...
                if (map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&MF_UWATER) uwater=1;
        }
...
//                      ch[cn].data[92]=TICKS*60;
                        ...
                        if (!it[in].active) {
                                if (it[in].temp==SK_RECALL && ch[cn].used==USE_ACTIVE) {
                                        ...
                                        old_inst=ch[cn].instance_id;

                                        if (!it[in].data[3]) {
                                                if (god_transfer_char(cn,it[in].data[0],it[in].data[1],-1)) {
                                                        if (!(ch[cn].flags&CF_INVISIBLE)) {
                                                                fx_add_effect(12,0,xo,yo,0,old_inst);
                                                                fx_add_effect(12,0,ch[cn].x,ch[cn].y,0,ch[cn].instance_id);
                                                        }
                                                }
                                        } else {
                                                // Leads to instance
                                                if (inst_isalive(it[in].data[4])) {
                                                        if (god_transfer_char(cn,it[in].data[0],it[in].data[1],it[in].data[4])) {
                                                                if (!(ch[cn].flags&CF_INVISIBLE)) {
                                                                        fx_add_effect(12,0,xo,yo,0,old_inst);
                                                                        fx_add_effect(12,0,ch[cn].x,ch[cn].y,0,ch[cn].instance_id);
                                                                }
                                                        ...
                                                        do_char_log(cn, 0, "The teleport effect fades. The instance it led to is no longer active.\n");
                                                }
                                        }
...
void do_depot_char(int cn,int co,int nr)
{
        ...
        inst_id = ch[cn].instance_id;
        if (inst_id == -1) {
                if (map[ch[cn].x + ch[cn].y * MAPX].flags&MF_BANK) inbank = 1;
        } else {
                if (map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&MF_BANK) inbank = 1;
        }

        if (!inbank && !(ch[cn].flags&CF_GOD)) {
	...
	if ((ch[co].flags&CF_BODY) && !lootflag) return;
		...
		if ((ch[cn].flags&CF_PLAYER) && !autoflag) {
			...
			if (ch[cn].data[71]>MAXSAY) {
                ...
                // Remove instance templates when looked at
                if (ch[co].flags&CF_INST_TEMP) {
                        do_char_log(cn, 1, "Removed instance template %s.\n", ch[co].name);
                        god_destroy_items(co);
                        ch[co].used = USE_EMPTY;
                        return;
                }
                ...
                if ((ch[co].flags&(CF_PLAYER)) && ch[co].data[14] && !(ch[co].flags&CF_GOD)) {
                        ...
                        do_char_log(cn,1,"%s died %d times, the last time on the day %d of the year %d, killed by %s %s.\n",
                                ...
                                killer,
                                get_area_m(ch[co].data[17]%MAPX,ch[co].data[17]/MAPX,1)); //TODO - data[17] is last killed position, should also handle instance area name
                }
...
// Player uses portal to join instance of associated area, inst_id=-1 is new area
void do_joininst(int cn, int inst_id)
{
        int in, x, y, m;

        if (!inst_isalive(inst_id) && inst_id != -1) {
                do_char_log(cn, 0, "That instance is no longer active.\n");
                return;
        }
        ...
        // Verify that player is standing/looking at item with driver 70, leading to given instance
        if (ch[cn].instance_id == -1) in = map[x+y*MAPX].it;
        else in = map_instancedtiles[ch[cn].instance_id][x+y*map_instances[ch[cn].instance_id].width].it;
        ...
        if (!in) {
                ...
                if (ch[cn].instance_id == -1) {
                        ...
                        m = x + y * map_instances[ch[cn].instance_id].width;
                        if (m > 0 && m < map_instances[ch[cn].instance_id].width * map_instances[ch[cn].instance_id].height) {
                                in = map_instancedtiles[ch[cn].instance_id][m].it;
                        }
                }
        }

        if (!in) {
                do_char_log(cn, 0, "You must be looking at/standing over an instance transporter to do that.\n");
                return;
        }

        if (it[in].used != USE_EMPTY && it[in].driver == 70) {
                int old_inst_id = ch[cn].instance_id;
                int old_chx = ch[cn].x;
                int old_chy = ch[cn].y;
                if (inst_id == -1) {
                        // Create new instance
                        if (map_instancebases[it[in].data[2]].used != USE_EMPTY) {
                                ...
                                map_instances[new_inst_id].owner = ch[cn].player;

                                if (!god_transfer_char(cn, it[in].data[0], it[in].data[1], new_inst_id)) {
                                        do_char_log(cn, 0, "Could not drop you in new instance.\n");
                                } else {
                                        fx_add_effect(6,0,old_chx,old_chy,0,old_inst_id);
                                        fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,new_inst_id);
                                }
                        }
                        ...
                        if (!god_transfer_char(cn, it[in].data[0], it[in].data[1], inst_id)) {
                                ...
                                fx_add_effect(6,0,old_chx,old_chy,0,old_inst_id);
                                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,inst_id);
                        }
                }
        }
}

static inline void map_add_light(int x,int y,int v,int inst_id)
{
        ...
        //              xlog("Error in light computations at %d,%d (+%d=%d).",x,y,v,map[m].light);
                ...
                m=x+y*map_instances[inst_id].width;

                map_instancedtiles[inst_id][m].light+=v;

                if (map_instancedtiles[inst_id][m].light<0) {
                        map_instancedtiles[inst_id][m].light=0;
                }
        }
}

void do_add_light(int xc,int yc,int stren,int inst_id)
{
        ...
        map_add_light(xc,yc,stren,inst_id);
        ...
        if (inst_id == -1) {
                ...
                xs=max(0,xc-LIGHTDIST);
                ys=max(0,yc-LIGHTDIST);
                xe=min(map_instances[inst_id].width-1,xc+1+LIGHTDIST);
                ye=min(map_instances[inst_id].height-1,yc+1+LIGHTDIST);
        }
        ...
        for (y=ys; y<ye; y++) {
                for (x=xs; x<xe; x++) {
                        ...
                        if ((v=can_see(0,xc,yc,x,y,LIGHTDIST,inst_id))!=0) {
                                d=stren/(v*(abs(xc-x)+abs(yc-y)));
                                if (flag) map_add_light(x,y,-d,inst_id);
                                else map_add_light(x,y,d,inst_id);
                        }
                }
        }
        prof_stop(9,prof);
...
int may_attack_msg(int cn, int co, int msg)
{
	...
	// player GC? act as if he would try to attack the master of the GC instead
        ...
        // Check for NOFIGHT
        char nf_cn=0, nf_co=0;
        if (inst_id == -1) {
                if (map[ch[cn].x + ch[cn].y * MAPX].flags&MF_NOFIGHT) nf_cn = 1;
                if (map[ch[co].x + ch[co].y * MAPX].flags&MF_NOFIGHT) nf_co = 1;
        } else {
                if (map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&MF_NOFIGHT) nf_cn = 1;
                if (map_instancedtiles[inst_id][ch[co].x + ch[co].y * map_instances[inst_id].width].flags&MF_NOFIGHT) nf_co = 1;
        }
        if (nf_cn || nf_co) {
        ...
        char af_cn=0, af_co=0;
        if (inst_id == -1) {
                if (map[ch[cn].x + ch[cn].y * MAPX].flags&MF_ARENA) af_cn = 1;
                if (map[ch[co].x + ch[co].y * MAPX].flags&MF_ARENA) af_co = 1;
        } else {
                if (map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&MF_ARENA) af_cn = 1;
                if (map_instancedtiles[inst_id][ch[co].x + ch[co].y * map_instances[inst_id].width].flags&MF_ARENA) af_co = 1;
        }
        if (af_cn && af_co) return 1;
...
void remember_pvp(int cn, int co)
{
        ...
        int inst_id = ch[cn].instance_id;
        if (inst_id == -1) {
                if (map[ch[cn].x + ch[cn].y * MAPX].flags&MF_ARENA) inarena = 1;
        } else {
                if (map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&MF_ARENA) inarena = 1;
        }
        if (inarena) return; // Arena attacks don't count
