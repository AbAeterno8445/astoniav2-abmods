...
#define INST_MAX                2000            // Maximum simultaneous alive instances
#define INST_MAXBASES           1000
#define INST_DURATION           (10*60*TICKS)   // Time instances remain alive after inactivity, in seconds

#define INST_BASES_SIZE         (sizeof(struct mapinstance)*INST_MAXBASES)
...
struct mapinstance
{
        ...
        char name[40];                  // name of map
        char fname[20];                 // file name of map (within .dat/areas/)

        int creation_time;              // time of creation, in format HHMM (creation_time/100 = hour, creation_time%100 = mins)
        int last_activity_time;         // globs->ticker at last moment of activity, used for instance expiration

        int mobs_remaining;             // Evil NPCs remaining in instance (alignment < 0) - also used for amount of characters in instance bases

} __attribute__ ((packed));

// Instance character templates
struct instcharacter
{
        short int x,y;
        unsigned char dir;
        int temp;               // Original character template
} __attribute__ ((packed));
...
struct character
{
        ...
        // map stuff
        short instance_id;        // current instance id. -1 for global map area
...
#define OLDCHARSIZE (sizeof(struct oldcharacter)*MAXCHARS)
#define OLDTCHARSIZE (sizeof(struct oldcharacter)*MAXTCHARS)
...
struct oldcharacter
{
        ...
        // map stuff
        int instance_id;        // current instance id. -1 for global map area
        short int x,y;          // current position x,y NOTE: x=-1, y=-1 = void
        short int tox,toy;      // target coordinated, where the char will be next turn
        short int frx,fry;      // where the char was last turn
        short int status;       // what the character is doing, animation-wise
...
extern struct map **map_instancedtiles;         // Map data for instanced areas
extern struct mapinstance *map_instancebases;   // Instance bases from which new instances are copied
extern struct mapinstance *map_instances;       // Alive instances
