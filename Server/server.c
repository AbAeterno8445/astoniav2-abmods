...
void new_player(int sock)
{
        ...
        ioctl(nsock,FIONBIO,(unsigned long*)&one);     // non-blocking mode
	...
	player[n].ticker_started=0;
        ...
        memset(player[n].cmap,0,sizeof(struct cmap)*RENDERDIST*RENDERDIST);
        memset(player[n].xmap,0,sizeof(player[n].xmap));
	memset(player[n].passwd,0,sizeof(player[n].passwd));

        for (m=0; m<RENDERDIST*RENDERDIST; m++) {
...
void tmplabcheck(int in)
{
	...
	if (ch[cn].temple_x!=HOME_MERCENARY_X && ch[cn].temple_x!=HOME_PURPLE_X && ch[cn].temple_x!=813) return;
...
int main(int argc,char *args[])
{
	...
	if (globs->flags&GF_DIRTY) {
        ...
        ioctl(sock,FIONBIO,(unsigned long*)&one);      // non-blocking mode
        ...
        for (n=1; n<MAXCHARS; n++) {
                if (ch[n].used==USE_ACTIVE) {
                        if (ch[n].flags&(CF_PLAYER|CF_USURP) && ch[n].instance_id != -1) {
                                // Gracefully remove player from previous instance
                                ch[n].instance_id = -1;
                                god_transfer_char(n, ch[n].temple_x, ch[n].temple_y, -1);
                        }
	...
	for (n=1; n<MAXTCHARS; n++) {
		...
		if (abs(x-ch_temp[n].x)+abs(y-ch_temp[n].y)>200) {
			xlog("RESET %d (%s) resting position: %d %d -> %d %d",n,ch_temp[n].name,ch_temp[n].x,ch_temp[n].y,x,y);
