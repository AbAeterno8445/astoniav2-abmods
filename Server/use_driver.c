...
int use_door(int cn,int in)
{
        ...
        inst_id = it[in].instance_id;

        if (inst_id == -1) {
                if (map[it[in].x+it[in].y*MAPX].ch) return 0;
        } else {
                if (map[it[in].x+it[in].y*map_instances[inst_id].width].ch) return 0;
        }
        ...
        reset_go(it[in].x,it[in].y,inst_id);
        add_lights(it[in].x,it[in].y,inst_id);
...
void finish_laby_teleport(int cn,int nr,int exp)
{
	...
	for (n=0; n<20; n++) {
        ...
        god_transfer_char(cn,HOME_MERCENARY_X,HOME_MERCENARY_Y,-1);
        fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
...
int teleport(int cn,int in)
{
        ...
        if (it[in].data[2]) {   // lab-solved teleport
                ...
                god_transfer_char(cn,it[in].data[0],it[in].data[1],ch[cn].instance_id);
                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,ch[cn].instance_id);
        }
...
int teleport2(int cn,int in2)
{
	...
	if (it[in2].data[2] && it[in2].data[2]+TICKS*60*4<globs->ticker) {
        ...
        it[in].data[3] = it[in2].data[3];
        it[in].data[4] = it[in2].data[4];
...
int use_labyrinth(int cn,int in)
{
        ...
        switch(ch[cn].data[20]) {
                case    0:      fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                flag=god_transfer_char(cn,64,56,-1);
                                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                break;
                case    1:      fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                flag=god_transfer_char(cn,95,207,-1);
                                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                break;
                case    2:      fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                flag=god_transfer_char(cn,74,240,-1);
                                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                break;
                case    3:      fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                flag=god_transfer_char(cn,37,370,-1);
                                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                break;
                case    4:      fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                flag=god_transfer_char(cn,114,390,-1);
                                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                break;
                case    5:      fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                flag=god_transfer_char(cn,28,493,-1);
                                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                break;
                case    6:      fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                flag=god_transfer_char(cn,24,534,-1);
                                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                break;
                case    7:      fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                flag=god_transfer_char(cn,118,667,-1);
                                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                break;
                case    8:      fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                flag=god_transfer_char(cn,63,720,-1);
                                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                break;
                case    9:      fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                flag=god_transfer_char(cn,33,597,-1);
                                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
                                break;
...
{
        god_transfer_char(cn,it[in].x+it[in].data[0],it[in].y+it[in].data[1],ch[cn].instance_id);
...
int use_crystal_sub(int cn,int in)
{
        ...
        if (!god_drop_char_fuzzy(cc,it[in].x,it[in].y,-1)) {
...
int use_mine_respawn(int cn,int in)
{
        ...
        cc=pop_create_char(tmp,0,-1);
        if (!god_drop_char_fuzzy(cc,it[in].x,it[in].y,ch[cn].instance_id)) {
...
int use_lever(int cn,int in)
{
        ...
        m=it[in].data[0];
        if (inst_id == -1) in2=map[m].it;
        else in2=map_instancedtiles[inst_id][m].it;
        ...
        if (it[in2].light[0]!=it[in2].light[1])
                do_add_light(it[in2].x,it[in2].y,it[in2].light[1]-it[in2].light[0],inst_id);
...
int use_spawn(int cn,int in)
{
        ...
        if ((temp=it[in].data[2])!=0)
                fx_add_effect(2,TICKS*10,ch_temp[temp].x,ch_temp[temp].y,temp,it[in].instance_id);
...
int use_pile(int cn,int in)
{
        ...
        if (!RANDOM(chance)) { // something there?
                ...
                if (it[in2].flags&IF_TAKE) {    // takeable object?
                        ...
                        god_drop_item(in2,x,y,-1);
                        fx_add_effect(9,16,in2,it[in2].data[0],0,inst_id);
                }
        }
        return 1;
}
...
int mine_wall(int in,int x,int y)
{
        ...
        if (!in) {
                if (inst_id == -1) in = map[x + y * MAPX].it;
                else in = map_instancedtiles[inst_id][x + y * map_instances[inst_id].width].it;
        }
        if (!in) return 0;

        if (it[in].data[3]) // add rebuild wall effect
                fx_add_effect(10,TICKS*60*15,it[in].x,it[in].y,it[in].temp,inst_id);
...
int mine_state(int x,int y,int inst_id)
{
        int in;
...
int use_mine(int cn,int in)
{
        ...
        if ((in2=ch[cn].worn[WN_RHAND])!=0) {
                ...
                if (inst_id_cn == -1) do_area_sound(cn,0,ch[cn].x,ch[cn].y,11);
                else do_area_sound_inst(inst_id_cn,cn,0,ch[cn].x,ch[cn].y,11);
        } else {
                str/=10;
                if (ch[cn].a_hp<10000) {
                        do_char_log(cn,0,"You need a proper tool to mine this wall.\n");
                        ch[cn].misc_action=DR_IDLE;
                        return 0;
                }
                ch[cn].a_hp-=500;
        ...
        if (tmp<=0) {
                reset_go(it[in].x,it[in].y,inst_id_cn);
                remove_lights(it[in].x,it[in].y,inst_id_cn);

                tmp=mine_wall(in,0,0)+1;

                reset_go(it[in].x,it[in].y,inst_id_cn);
                add_lights(it[in].x,it[in].y,inst_id_cn);
...
int use_mine_fast(int cn,int in)
{
        ...
        if (inst_id == -1) map[it[in].x + it[in].y * MAPX].it = 0;
        else map_instancedtiles[inst_id][it[in].x + it[in].y * map_instances[inst_id].width].it = 0;
        it[in].used=USE_EMPTY;
...
int use_gargoyle(int cn,int in)
{
        ...
        if (!god_drop_char_fuzzy(cc,ch[cn].x,ch[cn].y,inst_id)) {
                ch[cc].used=USE_EMPTY;
...
int spawn_penta_enemy(int in)
{
	...
	if (tmp>=22) {
                tmp-=22;
                if (tmp>3) tmp=3;
                cn=pop_create_char(1094+tmp,0,inst_id);
        } else if (tmp>17) {
                tmp-=17;
                if (tmp>4) tmp=4;
                cn=pop_create_char(538+tmp,0,inst_id);
        } else cn=pop_create_char(364+tmp,0,inst_id);
        ...
        if (!god_drop_char_fuzzy(cn,it[in].x,it[in].y,inst_id)) {
        ...
        for (n=1; n<MAXITEM; n++) {
                ...
                if (it[n].active==0) {
                        if (it[n].light[0]!=it[n].light[1] && it[n].x>0) {
                                do_add_light(it[n].x,it[n].y,it[n].light[1]-it[n].light[0],it[n].instance_id);
                        }
                }
...
int use_kill_undead(int cn,int in)
{
        ...
        fx_add_effect(7,0,ch[cn].x,ch[cn].y,0,inst_id);
        ...
        if (inst_id == -1) {
                        ...
                        for (x=ch[cn].x-8; x<ch[cn].x+8; x++) {
                                ...
                                if ((co=map[x+y*MAPX].ch)!=0) {
                                        if (ch[co].flags&CF_UNDEAD) {
                                                do_hurt(cn,co,500,2);
                                                fx_add_effect(5,0,ch[co].x,ch[co].y,0,ch[co].instance_id);
                                        }
                                }
                        }
                }
                ...
                for (y=ch[cn].y-8; y<ch[cn].y+8; y++) {
                        if (y<1 || y>=map_instances[inst_id].height) continue;
                        for (x=ch[cn].x-8; x<ch[cn].x+8; x++) {
                                if (x<1 || x>=map_instances[inst_id].width) continue;
                                if ((co=map_instancedtiles[inst_id][x+y*map_instances[inst_id].width].ch)!=0) {
                                        if (ch[co].flags&CF_UNDEAD) {
                                                do_hurt(cn,co,500,2);
                                                fx_add_effect(5,0,ch[co].x,ch[co].y,0,ch[co].instance_id);
                                        }
...
int teleport3(int cn,int in)    // out of labyrinth
{
        ...
        fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
        god_transfer_char(cn,it[in].data[0],it[in].data[1],-1);
        fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
...
int teleport_inst_prompt(int cn,int in)
{
        int nr, inst, base_id, n_ind;
        char buf[16];
        ...
        for (int i=0; i<7; i++) {
                player[nr].inst_buffer[i] = -1;
        }

        // Send instance name (pt 1)
        buf[0]=SV_AREAINST;
        *(unsigned char*)(buf+1)=0;
        for (n_ind=2; n_ind<15; n_ind++) buf[n_ind]=map_instancebases[base_id].name[n_ind-2];
        xsend(nr,buf,16);

        // Send instance name (pt 2)
        if (strlen(map_instancebases[base_id].name) > 12) {
                buf[0]=SV_AREAINST;
                *(unsigned char*)(buf+1)=1;
                for (n_ind=2; n_ind<15; n_ind++) buf[n_ind]=map_instancebases[base_id].name[n_ind+8];
                xsend(nr,buf,16);
        }
        ...
        for (int i=0; i<INST_MAX; i++) {
                ...
                *(unsigned char*)(buf+1)=3;
                ...
                // Send time left in seconds
                *(unsigned short*)(buf+2)=max(0, (INST_DURATION - (globs->ticker - map_instances[i].last_activity_time)) / TICKS);
                // Send creation time
                *(unsigned short*)(buf+4)=map_instances[i].creation_time;
                // Monsters remaining
                *(unsigned short*)(buf+6)=min(51,map_instances[i].mobs_remaining);
                xsend(nr,buf,8);

                player[nr].inst_buffer[inst] = i;

                inst++;
                if (inst >= 7) break;
        }

        // Finally display instance manager screen
        buf[0]=SV_AREAINST;
...
int use_create_npc(int cn,int in)
{
        ...
        co=pop_create_char(it[in].data[0],0,inst_id);
        if (!co) return 0;

        if (!god_drop_char_fuzzy(co,it[in].x,it[in].y,inst_id)) {
...
int use_lab8_moneyshrine(int cn, int in)
{
        ...
        ch[cn].citem = 0;
        god_transfer_char(cn,it[in].data[1],it[in].data[2],-1);
        if (ch[cn].a_mana<ch[cn].mana[5]*1000) {
                ch[cn].a_mana=ch[cn].mana[5]*1000;
                do_char_log(cn,0,"You feel the hand of the Goddess of Magic touch your mind.\n");
                fx_add_effect(6,0,ch[cn].x,ch[cn].y,0,-1);
...
void use_driver(int cn,int in,int carried)
{
        ...
        if (!carried && cn > 0 && inst_id != inst_id_cn) return;
        ...
        if ((it[in].flags&IF_USE) || !cn) {
                ...
                if (!carried) {
                        if (inst_id == -1) {
                                ...
                                if ((map[m].ch || map[m].to_ch) && it[in].driver != 33) return;
                        } else {
                                m = it[in].x + it[in].y * map_instances[inst_id].width;
                                if ((map_instancedtiles[inst_id][m].ch || map_instancedtiles[inst_id][m].to_ch) && it[in].driver != 33) return;
                        }
                }
                ...
                if (it[in].active) {
                        if (it[in].flags&IF_USEDEACTIVATE) {
                                ...
                                if (it[in].light[0]!=it[in].light[1] && it[in].x>0)
                                        do_add_light(it[in].x,it[in].y,it[in].light[0]-it[in].light[1],inst_id);
                                if (carried) {
                                        it[in].flags|=IF_UPDATE;
                                        do_update_char(cn);
                                }
                        ...
                        if (it[in].flags&IF_USEACTIVATE) {
                                ...
                                if (it[in].light[0]!=it[in].light[1] && it[in].x>0)
                                        do_add_light(it[in].x,it[in].y,it[in].light[1]-it[in].light[0],inst_id);
                                if (carried) {
                                        it[in].flags|=IF_UPDATE;
                                        do_update_char(cn);
                                }
...
void lightage(int in,int multi)
{
        ...
        if (inst_id == -1) {
                ...
                m_wid = map_instances[inst_id].height;
        }

        if ((cn=it[in].carried)!=0) m=ch[cn].x+ch[cn].y*m_wid;
        else m=it[in].x+it[in].y*m_wid;

        if (m<1 || m>=m_wid*m_hei) return;

        if (inst_id == -1) light=map[m].light;
        else light=map_instancedtiles[inst_id][m].light;
        if (light<1) return;
...
int may_deactivate(int in)
{
        ...
        for (n=1; n<10; n++) {
                ...
                if (inst_id == -1) {
                        ...
                        if ((in2=map_instancedtiles[inst_id][m].it)==0) return 0;
                }
                if (it[in2].driver!=26) return 0;
        }
        return 1;
...
void spiderweb(int in)
{
        ...
        for (n=1; n<4; n++) {
                if ((cn=it[in].data[n])==0 || ch[cn].data[0]!=in || ch[cn].used==USE_EMPTY || (ch[cn].flags&CF_BODY)) {
                        cn=pop_create_char(390+RANDOM(3),0,inst_id);
                        if (!cn) continue;
                        ch[cn].flags&=~CF_RESPAWN;
                        ch[cn].data[0]=in;
                        if (inst_id == -1) ch[cn].data[29] = it[in].x + it[in].y * MAPX;
                        else ch[cn].data[29] = it[in].x + it[in].y * map_instances[inst_id].width;
                        ch[cn].data[60]=TICKS*60*2;
                        ch[cn].data[73]=8;
                        ch[cn].dir=1;
                        if (!god_drop_char_fuzzy(cn,it[in].x,it[in].y,inst_id)) {
...
void greenlingball(int in)
{
        ...
        for (n=1; n<4; n++) {
                if ((cn=it[in].data[n])==0 || ch[cn].data[0]!=in || ch[cn].used==USE_EMPTY || (ch[cn].flags&CF_BODY)) {
                        cn=pop_create_char(553+it[in].data[0],0,inst_id);
                        if (!cn) continue;
                        ch[cn].flags&=~CF_RESPAWN;
                        ch[cn].data[0]=in;
                        if (inst_id == -1) ch[cn].data[29] = it[in].x + it[in].y * MAPX;
...
void item_tick_expire(void)
{
        ...
        // Item expiration in instances
        item_tick_expire_inst();

        // Item expiration in main map
        for (x=0,m=y*MAPX; x<MAPX; x++,m++) {
                if ((in=map[m].it)!=0) {
                        if ((it[in].flags&IF_REACTIVATE) && !it[in].active) {
                                if (!map[m].ch && !map[m].to_ch) {
                                        ...
                                        if (it[in].light[0]!=it[in].light[1])
                                                do_add_light(x,y,it[in].light[1]-it[in].light[0],-1);
                                }
                        }
                        // active and expire
                        if (it[in].active && it[in].active!=0xffffffff) {
                                if (it[in].active<=EXP_TIME) {
                                        if (may_deactivate(in) && ((!map[m].ch && !map[m].to_ch) || it[in].driver == 33)) {
                                                use_driver(0,in,0);
                                                it[in].active=0;
                                                if (it[in].light[0]!=it[in].light[1])
                                                        do_add_light(x,y,it[in].light[0]-it[in].light[1],-1);
                                        }
                        ...
                        if (item_age(in) && it[in].damage_state==5) {
                                if (it[in].light[act]) do_add_light(x,y,-it[in].light[act],-1);
                                map[m].it=0;
                                it[in].used=USE_EMPTY;
                                globs->expire_cnt++;
						...
						if (temp==189 || temp==561) {
							fx_add_effect(2,TICKS*60*20+RANDOM(TICKS*60*5),ch_temp[temp].x,ch_temp[temp].y,temp,-1);
						} else {
							fx_add_effect(2,TICKS*60*1+RANDOM(TICKS*60*1),ch_temp[temp].x,ch_temp[temp].y,temp,-1);
						}
...
void trap2(int cn,int tmp)
{
        ...
        cc=pop_create_char(tmp,0,ch[cn].instance_id);
        if (!god_drop_char_fuzzy(cc,ch[cn].x,ch[cn].y,ch[cn].instance_id)) {
                xlog("drop failed");
                ch[cc].used=USE_EMPTY;
                return;
        }
...
void start_trap(int cn,int in)
{
        if (it[in].duration) {
                it[in].active=it[in].duration;
                if (it[in].light[0]!=it[in].light[1] && it[in].x>0)
                        do_add_light(it[in].x,it[in].y,it[in].light[1]-it[in].light[0],it[in].instance_id);
        }
...
void step_trap_remove(int cn,int in)
{
        if (it[in].active) {
                it[in].active=0;
                if (it[in].light[0]!=it[in].light[1] && it[in].x>0)
                        do_add_light(it[in].x,it[in].y,it[in].light[0]-it[in].light[1],it[in].instance_id);
        }
}
...
int step_portal_arena(int cn,int in)
{
        ...
        co=pop_create_char(nr,0,-1);
        if (!god_drop_char_fuzzy(co,it[in].data[0]%MAPX,it[in].data[0]/MAPX,-1)) {
...
int step_teleport(int cn, int in)
{
        ...
        int loc_off[] = { 0, -m_wid, m_hei, 1, -1 };

        x = it[in].data[0];
        y = it[in].data[1];
        
        if (x < 0 || x >= m_wid || y < 0 || y >= m_hei) {
        ...
        for (j=0; j<ARRAYSIZE(loc_off); j++) {
                ...
                if (inst_id == -1) {
                        ...
                        if (map_instancedtiles[inst_id][m2].flags&MF_MOVEBLOCK) continue;
                        if (map_instancedtiles[inst_id][m2].ch) continue;
                        if (map_instancedtiles[inst_id][m2].to_ch) continue;
                        if ((in=map_instancedtiles[inst_id][m2].it)!=0 && (it[in].flags&IF_MOVEBLOCK)) continue;
                        if (map_instancedtiles[inst_id][m2].flags&(MF_TAVERN|MF_DEATHTRAP)) continue;
                }
                m3 = m2; break;
        }
        if (m3 == 0) {
        ...
        if (inst_id == -1) {
                ...
                map_instancedtiles[inst_id][m3].ch=cn;
                map_instancedtiles[inst_id][m3].to_ch=0;
        }
        ch[cn].x = m3 % m_wid;
        ch[cn].y = m3 / m_wid;
        do_area_notify(cn,0,ch[cn].x,ch[cn].y,NT_SEE,cn,0,0,0);
