	...
	for (y=m=0; y<MAPY; y++) {
		...
		for (x=0; x<MAPX; x++,m++) {
			if (map[m].flags&MF_INDOORS) {
				cnt2++;
				compute_dlight(x,y,-1);
			}
			if ((in=map[m].it)==0) continue;
			if (it[in].active) {
				if (it[in].light[1]) {
					do_add_light(x,y,it[in].light[1],-1);
					cnt1++;
				}
			} else {
				if (it[in].light[0]) {
					do_add_light(x,y,it[in].light[0],-1);
					cnt1++;
				}
			}
		}
...
void init_lights_inst(int inst_id)
{
	int x,y,in,m,cnt1=0,cnt2=0;

	for (y=m=0; y<map_instances[inst_id].height; y++) {
		for (x=0; x<map_instances[inst_id].width; x++,m++) {
	...
	for (y=m=0; y<map_instances[inst_id].height; y++) {
		for (x=0; x<map_instances[inst_id].width; x++,m++) {
			...
			if ((in=map_instancedtiles[inst_id][m].it)==0) continue;
			if (it[in].active) {
				if (it[in].light[1]) {
...
int pop_create_char(int n,int drop,int inst_id)
{
...
/*	--end	*/
	...
	if (drop) {
		if (!god_drop_char(cn,ch[cn].x,ch[cn].y,inst_id)) {
			printf("Could not drop char %d (%s) at (%d;%d)\n",n,ch[cn].name,ch[cn].x,ch[cn].y);
			god_destroy_items(cn);
			ch[cn].used=USE_EMPTY;
			return 0;
		}
...
void reset_char(int n)
{
	...
	if (ch_temp[n].used==USE_ACTIVE) { // schedule respawn
		fx_add_effect(2,TICKS*10,ch_temp[n].x,ch_temp[n].y,n,-1);
	}
}
		...
		if (it[in].temp==n) {
			...
			if ((it_temp[n].flags&(IF_TAKE|IF_LOOK|IF_LOOKSPECIAL|IF_USE|IF_USESPECIAL)) || it[in].carried) {
				...
				if (it[in].instance_id == -1) {
					...
					map_instancedtiles[inst_id][it[in].x+it[in].y*map_instances[inst_id].width].it=0;
					it[in].used=USE_EMPTY;
					map_instancedtiles[inst_id][it[in].x+it[in].y*map_instances[inst_id].width].fsprite=it_temp[n].sprite[0];
					if (it_temp[n].flags&IF_MOVEBLOCK) map_instancedtiles[inst_id][it[in].x+it[in].y*map_instances[inst_id].width].flags|=MF_MOVEBLOCK;
					if (it_temp[n].flags&IF_SIGHTBLOCK) map_instancedtiles[inst_id][it[in].x+it[in].y*map_instances[inst_id].width].flags|=MF_SIGHTBLOCK;
				}
			}
		}
	}
...
void pop_wipe(void)
{
	...
	for (n=1; n<MAXITEM; n++) {
		...
		if (it[n].used==USE_ACTIVE) {
			if (it[n].instance_id == -1) map[it[n].x+it[n].y*MAPX].it=0;
			else map[it[n].x+it[n].y*map_instances[it[n].instance_id].width].it=0;
		}
		it[n].used=USE_EMPTY;
	}
