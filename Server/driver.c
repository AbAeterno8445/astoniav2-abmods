...
int npc_add_enemy(int cn,int co,int always)
{
        ...
        if (inst_id == -1) ch[cn].data[76] = ch[co].x + ch[co].y * MAPX;
        else ch[cn].data[76] = ch[co].x + ch[co].y * MAPX;
...
int npc_gotattack(int cn,int co,int dam)
{
        ...
        if (co && (ch[co].flags&CF_PLAYER) && ch[cn].alignment==10000 && (strcmp(ch[cn].name,"Peacekeeper") || ch[cn].a_hp<ch[cn].hp[5]*500) && ch[cn].data[70]<globs->ticker) {
                ...
                if (!god_drop_char_fuzzy(cc,ch[co].x,ch[co].y,ch[co].instance_id)) {
...
int npc_give(int cn,int co,int in,int money)
{
        ...
        if (in && ch[cn].data[49]==it[in].temp) {
                if (ch[cn].data[49]==740 && ch[cn].temp==518) { // hack for black candle
                        ...
                        if (ch[cn].instance_id == -1) do_area_log(cn,0,ch[cn].x,ch[cn].y,1,"The Cityguard is impressed by %s's deed.\n",ch[co].name);
                        else do_area_log_inst(ch[cn].instance_id,cn,0,ch[cn].x,ch[cn].y,1,"The Cityguard is impressed by %s's deed.\n",ch[co].name);
...
int npc_see(int cn,int co)
{
        ...
        if (inst_id == -1) {
                ...
                if (map[ch[cn].x + ch[cn].y * MAPX].flags&MF_INDOORS) indoor2=1;
                else indoor2=0;
        } else {
                if (map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&MF_INDOORS) indoor1=1;
                else indoor1=0;
                if (map_instancedtiles[inst_id][ch[cn].x + ch[cn].y * map_instances[inst_id].width].flags&MF_INDOORS) indoor2=1;
                else indoor2=0;
        }
                ...
                if (n==47) {
                        if (ch[cn].data[95]==2 && ch[cn].data[93]) { // attack distance
                                if (inst_id == -1)
                                        ...
                                        dist=max(abs((ch[cn].data[29]%map_instances[inst_id].width)-ch[co].x),abs((ch[cn].data[29]/map_instances[inst_id].width)-ch[co].y));
        ...
        if (ch[cn].data[95]==1 && (ch[co].flags&(CF_PLAYER)) && globs->ticker>ch[cn].data[27]+TICKS*120) {
                ...
                if (inst_id == -1) {
                        ...
                        x2=ch[cn].data[29]%map_instances[inst_id].width;
                        y2=ch[cn].data[29]/map_instances[inst_id].height;
                }
...
int npc_shout(int cn,int co,int code,int x,int y)
{
        if (ch[cn].data[53] && ch[cn].data[53]==code) { // someone called help. If it's our code, we come to the rescue
                ch[cn].data[92]=TICKS*60;
                if (ch[cn].instance_id == -1) ch[cn].data[54]=x+y*MAPX;
                else ch[cn].data[54]=x+y*map_instances[ch[cn].instance_id].width;
                ch[cn].data[55]=globs->ticker;
	...
	mana=ch[cn].a_mana/1000;
        ...
        if (n==20) {
                ...
                if (mana>=get_spellcost(cn,spell) && !(ch[co].data[96]&tmp)) {
                        ...
                        ch[co].data[96]|=tmp;
                        fx_add_effect(11,8,co,tmp,0,ch[co].instance_id);
                        return 1;
                }
        }
...
int npc_driver_high(int cn)
{
        ...
        // create light
        if (inst_id == -1) {
                if (ch[cn].data[62]>ch[cn].data[58] && check_dlight(ch[cn].x,ch[cn].y,ch[cn].instance_id)<20 && map[ch[cn].x+ch[cn].y*MAPX].light<20) {
                        if (npc_try_spell(cn,cn,SK_LIGHT)) return 1;
                }
        } else {
                if (ch[cn].data[62]>ch[cn].data[58] && check_dlight(ch[cn].x,ch[cn].y,ch[cn].instance_id)<20 &&
                map_instancedtiles[inst_id][ch[cn].x+ch[cn].y*map_instances[inst_id].width].light<20) {
                        if (npc_try_spell(cn,cn,SK_LIGHT)) return 1;
                }
        }
        ...
        if (inst_id == -1) {
                ...
                for (y=max(ch[cn].y-8,1); y<min(ch[cn].y+8,map_instances[inst_id].height-1); y++) {
                        for (x=max(ch[cn].x-8,1); x<min(ch[cn].x+8,map_instances[inst_id].width-1); x++) {
                                if ((in=map_instancedtiles[inst_id][x+y*map_instances[inst_id].width].it)!=0) {
                                        if (map_instancedtiles[inst_id][x+y*map_instances[inst_id].width].flags&MF_INDOORS) indoor2=1;
                                        else indoor2=0;
...
int shiva_activate_candle(int cn,int in)
{
        ...
        if (it[in].light[0]!=it[in].light[1] && it[in].x>0)
                do_add_light(it[in].x,it[in].y,it[in].light[0]-it[in].light[1],ch[cn].instance_id);

        fx_add_effect(6,0,it[in].x,it[in].y,0,ch[cn].instance_id);
        fx_add_effect(7,0,ch[cn].x,ch[cn].y,0,ch[cn].instance_id);
	...
	// are we supposed to loot graves?
        ...
        if (ch[cn].data[55] && ch[cn].data[55]+TICKS*120>globs->ticker && ch[cn].data[54]) {
                ...
                if (inst_id == -1) {
                        ...
                        ch[cn].goto_x=m%map_instances[inst_id].width+get_frust_x_off(globs->ticker);
                        ch[cn].goto_y=m/map_instances[inst_id].width+get_frust_y_off(globs->ticker);
                }
                ch[cn].data[58]=2;
                return;
        }
        ...
        if (ch[cn].data[77] && ch[cn].data[77]+TICKS*30>globs->ticker) {
                ...
                if (inst_id == -1) {
                        ...
                        ch[cn].goto_x=m%map_instances[inst_id].width+get_frust_x_off(ch[cn].data[36]);
                        ch[cn].goto_y=m/map_instances[inst_id].width+get_frust_y_off(ch[cn].data[36]);
                }
                return;
        }
        ...
        for (n=20; n<24; n++) { // close door, medium prio
                if ((m=ch[cn].data[n])!=0) {
                        ...
                        if (inst_id == -1) {
                                ...
                                !map_instancedtiles[inst_id][m+1].ch && !map_instancedtiles[inst_id][m+1].to_ch &&
                                !map_instancedtiles[inst_id][m-1].ch && !map_instancedtiles[inst_id][m-1].to_ch &&
                                !map_instancedtiles[inst_id][m+map_instances[inst_id].width].ch && !map_instancedtiles[inst_id][m+map_instances[inst_id].width].to_ch &&
                                !map_instancedtiles[inst_id][m-map_instances[inst_id].width].ch && !map_instancedtiles[inst_id][m-map_instances[inst_id].width].to_ch) {
                                        in=map_instancedtiles[inst_id][m].it;
                                        if (in && it[in].active) {
                                                ch[cn].misc_action=DR_USE;
                                                ch[cn].misc_target1=m%map_instances[inst_id].width;
                                                ch[cn].misc_target2=m/map_instances[inst_id].width;
                                                ch[cn].data[58]=1;
                                                return;
                                        }
                                }
        ...
        for (n=32; n<36; n++) { // activate light, medium prio
                if (inst_id == -1) {
                        ...
                        if ((m=ch[cn].data[n])!=0 && m<map_instances[inst_id].width*map_instances[inst_id].height) {
                                in=map_instancedtiles[inst_id][m].it;
                                if (in && !it[in].active) {
                                        ch[cn].misc_action=DR_USE;
                                        ch[cn].misc_target1=m%map_instances[inst_id].width;
        ...
        if (ch[cn].data[10]) {  // patrol, low
                ...
                if (inst_id == -1) {
                        ...
                        x=m%map_instances[inst_id].width+get_frust_x_off(ch[cn].data[36]);
                        y=m/map_instances[inst_id].width+get_frust_x_off(ch[cn].data[36]);
                }
        ...
        if (ch[cn].data[60]) { //random walk, low
                ...
                if (ch[cn].data[61]<1) {
                        ...
                        for (panic=0; panic<5; panic++) {
                                ...
                                if (inst_id == -1) {
                                        if (x<1 || x>=MAPX || y<1 || y>MAPY) continue;
                                } else {
                                        if (x<1 || x>=map_instances[inst_id].width || y<1 || y>map_instances[inst_id].height) continue;
                                }
                                ...
                                if (ch[cn].data[73]) { // too far away from origin?
                                        ...
                                        if (inst_id == -1) {
                                                ...
                                                xo=ch[cn].data[29]%map_instances[inst_id].width;
                                                yo=ch[cn].data[29]/map_instances[inst_id].height;
                                        }
        ...
        if (ch[cn].data[29]) {  // resting position, lowest prio
		        ...
		        x=m%MAPX+get_frust_x_off(ch[cn].data[36]);
                        ...
                        x=m%map_instances[inst_id].width+get_frust_x_off(ch[cn].data[36]);
                        y=m/map_instances[inst_id].width+get_frust_y_off(ch[cn].data[36]);
                }
        ...
        if (ch[cn].data[26]==2 && ch[cn].a_mana>ch[cn].mana[5]*900) {
                ...
                if (m<15) {
                        ...
                        if (n) {
                                for (m=0; m<n; m++) {
                                        co=pop_create_char(503+m,0,ch[cn].instance_id);
                                        if (!co) { do_sayx(cn,"create char (%d)",m); break; }
                                        if (!god_drop_char_fuzzy(co,452,345,ch[cn].instance_id)) {
