...
int god_drop_item(int nr,int x,int y,int inst_id)
{
        int m;
        ...
        if (inst_id == -1) {
                ...
                if (x < 0 || x >= map_instances[inst_id].width || y < 0 || y >= map_instances[inst_id].height) return 0;
        }
        ...
        if (inst_id == -1) {
                ...
                if (map_instancedtiles[inst_id][m].ch || map_instancedtiles[inst_id][m].to_ch || 
                map_instancedtiles[inst_id][m].it || (map_instancedtiles[inst_id][m].flags&(MF_MOVEBLOCK|MF_DEATHTRAP)) || 
                map_instancedtiles[inst_id][m].fsprite) return 0;

                map_instancedtiles[inst_id][m].it=nr;
        }

        it[nr].instance_id=inst_id;
        it[nr].x=(short)x;
        it[nr].y=(short)y;
        it[nr].carried=0;
...
int god_drop_item_fuzzy(int nr,int x,int y,int inst_id)
{
        if (god_drop_item(nr,x,y,inst_id)) return 1;

        if (can_go(x,y,x+1,y,inst_id) && god_drop_item(nr,x+1,y,inst_id)) return 1;
        if (can_go(x,y,x-1,y,inst_id) && god_drop_item(nr,x-1,y,inst_id)) return 1;
        if (can_go(x,y,x,y+1,inst_id) && god_drop_item(nr,x,y+1,inst_id)) return 1;
        if (can_go(x,y,x,y-1,inst_id) && god_drop_item(nr,x,y-1,inst_id)) return 1;

        if (can_go(x,y,x+1,y+1,inst_id) && god_drop_item(nr,x+1,y+1,inst_id)) return 1;
        if (can_go(x,y,x+1,y-1,inst_id) && god_drop_item(nr,x+1,y-1,inst_id)) return 1;
        if (can_go(x,y,x-1,y+1,inst_id) && god_drop_item(nr,x-1,y+1,inst_id)) return 1;
        if (can_go(x,y,x-1,y-1,inst_id) && god_drop_item(nr,x-1,y-1,inst_id)) return 1;

        if (can_go(x,y,x+2,y-2,inst_id) && god_drop_item(nr,x+2,y-2,inst_id)) return 1;
        if (can_go(x,y,x+2,y-1,inst_id) && god_drop_item(nr,x+2,y-1,inst_id)) return 1;
        if (can_go(x,y,x+2,y+0,inst_id) && god_drop_item(nr,x+2,y+0,inst_id)) return 1;
        if (can_go(x,y,x+2,y+1,inst_id) && god_drop_item(nr,x+2,y+1,inst_id)) return 1;
        if (can_go(x,y,x+2,y+2,inst_id) && god_drop_item(nr,x+2,y+2,inst_id)) return 1;
        if (can_go(x,y,x-2,y-2,inst_id) && god_drop_item(nr,x-2,y-2,inst_id)) return 1;
        if (can_go(x,y,x-2,y-1,inst_id) && god_drop_item(nr,x-2,y-1,inst_id)) return 1;
        ...
        if (can_go(x,y,x-2,y+1,inst_id) && god_drop_item(nr,x-2,y+1,inst_id)) return 1;
        if (can_go(x,y,x-2,y+2,inst_id) && god_drop_item(nr,x-2,y+2,inst_id)) return 1;
        if (can_go(x,y,x-1,y+2,inst_id) && god_drop_item(nr,x-1,y+2,inst_id)) return 1;
        if (can_go(x,y,x+0,y+2,inst_id) && god_drop_item(nr,x+0,y+2,inst_id)) return 1;
        if (can_go(x,y,x+1,y+2,inst_id) && god_drop_item(nr,x+1,y+2,inst_id)) return 1;
        if (can_go(x,y,x-1,y-2,inst_id) && god_drop_item(nr,x-1,y-2,inst_id)) return 1;
        if (can_go(x,y,x+0,y-2,inst_id) && god_drop_item(nr,x+0,y-2,inst_id)) return 1;
        if (can_go(x,y,x+1,y-2,inst_id) && god_drop_item(nr,x+1,y-2,inst_id)) return 1;
...
int god_drop_char(int cn,int x,int y,int inst_id)
{
        int m,in;
        ...
        if (inst_id == -1) {
                ...
                if (x<1 || x>map_instances[inst_id].width-2 || y<1 || y>map_instances[inst_id].height-2) return 0;

                m=x+y*map_instances[inst_id].width;
                if (map_instancedtiles[inst_id][m].ch || map_instancedtiles[inst_id][m].to_ch || ((in=map_instancedtiles[inst_id][m].it)!=0 && (it[in].flags&IF_MOVEBLOCK)) || (map_instancedtiles[inst_id][m].flags&MF_MOVEBLOCK) ||
                (map_instancedtiles[inst_id][m].flags&MF_TAVERN) || (map_instancedtiles[inst_id][m].flags&MF_DEATHTRAP)) {
                        return 0;
                }
        }
...
int god_drop_char_fuzzy(int nr,int x,int y,int inst_id)
{
        if (god_drop_char(nr,x,y,inst_id)) return 1;

        if (can_go(x,y,x+1,y,inst_id) && god_drop_char(nr,x+1,y,inst_id)) return 1;
        if (can_go(x,y,x-1,y,inst_id) && god_drop_char(nr,x-1,y,inst_id)) return 1;
        if (can_go(x,y,x,y+1,inst_id) && god_drop_char(nr,x,y+1,inst_id)) return 1;
        if (can_go(x,y,x,y-1,inst_id) && god_drop_char(nr,x,y-1,inst_id)) return 1;

        if (can_go(x,y,x+1,y+1,inst_id) && god_drop_char(nr,x+1,y+1,inst_id)) return 1;
        if (can_go(x,y,x+1,y-1,inst_id) && god_drop_char(nr,x+1,y-1,inst_id)) return 1;
        if (can_go(x,y,x-1,y+1,inst_id) && god_drop_char(nr,x-1,y+1,inst_id)) return 1;
        if (can_go(x,y,x-1,y-1,inst_id) && god_drop_char(nr,x-1,y-1,inst_id)) return 1;

        if (can_go(x,y,x+2,y-2,inst_id) && god_drop_char(nr,x+2,y-2,inst_id)) return 1;
        if (can_go(x,y,x+2,y-1,inst_id) && god_drop_char(nr,x+2,y-1,inst_id)) return 1;
        if (can_go(x,y,x+2,y+0,inst_id) && god_drop_char(nr,x+2,y+0,inst_id)) return 1;
        if (can_go(x,y,x+2,y+1,inst_id) && god_drop_char(nr,x+2,y+1,inst_id)) return 1;
        if (can_go(x,y,x+2,y+2,inst_id) && god_drop_char(nr,x+2,y+2,inst_id)) return 1;
        if (can_go(x,y,x-2,y-2,inst_id) && god_drop_char(nr,x-2,y-2,inst_id)) return 1;
        if (can_go(x,y,x-2,y-1,inst_id) && god_drop_char(nr,x-2,y-1,inst_id)) return 1;
        ...
        if (can_go(x,y,x-2,y+1,inst_id) && god_drop_char(nr,x-2,y+1,inst_id)) return 1;
        if (can_go(x,y,x-2,y+2,inst_id) && god_drop_char(nr,x-2,y+2,inst_id)) return 1;
        if (can_go(x,y,x-1,y+2,inst_id) && god_drop_char(nr,x-1,y+2,inst_id)) return 1;
        if (can_go(x,y,x+0,y+2,inst_id) && god_drop_char(nr,x+0,y+2,inst_id)) return 1;
        if (can_go(x,y,x+1,y+2,inst_id) && god_drop_char(nr,x+1,y+2,inst_id)) return 1;
        if (can_go(x,y,x-1,y-2,inst_id) && god_drop_char(nr,x-1,y-2,inst_id)) return 1;
        if (can_go(x,y,x+0,y-2,inst_id) && god_drop_char(nr,x+0,y-2,inst_id)) return 1;
        if (can_go(x,y,x+1,y-2,inst_id) && god_drop_char(nr,x+1,y-2,inst_id)) return 1;
...
int god_drop_char_fuzzy_large(int nr,int x,int y,int xs,int ys,int inst_id)
{
        if (can_go(xs,ys,x,y,inst_id) && god_drop_char(nr,x,y,inst_id)) return 1;

        if (can_go(xs,ys,x+1,y,inst_id) && god_drop_char(nr,x+1,y,inst_id)) return 1;
        if (can_go(xs,ys,x-1,y,inst_id) && god_drop_char(nr,x-1,y,inst_id)) return 1;
        if (can_go(xs,ys,x,y+1,inst_id) && god_drop_char(nr,x,y+1,inst_id)) return 1;
        if (can_go(xs,ys,x,y-1,inst_id) && god_drop_char(nr,x,y-1,inst_id)) return 1;

        if (can_go(xs,ys,x+1,y+1,inst_id) && god_drop_char(nr,x+1,y+1,inst_id)) return 1;
        if (can_go(xs,ys,x+1,y-1,inst_id) && god_drop_char(nr,x+1,y-1,inst_id)) return 1;
        if (can_go(xs,ys,x-1,y+1,inst_id) && god_drop_char(nr,x-1,y+1,inst_id)) return 1;
        if (can_go(xs,ys,x-1,y-1,inst_id) && god_drop_char(nr,x-1,y-1,inst_id)) return 1;

        if (can_go(xs,ys,x+2,y-2,inst_id) && god_drop_char(nr,x+2,y-2,inst_id)) return 1;
        if (can_go(xs,ys,x+2,y-1,inst_id) && god_drop_char(nr,x+2,y-1,inst_id)) return 1;
        if (can_go(xs,ys,x+2,y+0,inst_id) && god_drop_char(nr,x+2,y+0,inst_id)) return 1;
        if (can_go(xs,ys,x+2,y+1,inst_id) && god_drop_char(nr,x+2,y+1,inst_id)) return 1;
        if (can_go(xs,ys,x+2,y+2,inst_id) && god_drop_char(nr,x+2,y+2,inst_id)) return 1;
        if (can_go(xs,ys,x-2,y-2,inst_id) && god_drop_char(nr,x-2,y-2,inst_id)) return 1;
        if (can_go(xs,ys,x-2,y-1,inst_id) && god_drop_char(nr,x-2,y-1,inst_id)) return 1;
        ...
        if (can_go(xs,ys,x-2,y+1,inst_id) && god_drop_char(nr,x-2,y+1,inst_id)) return 1;
        if (can_go(xs,ys,x-2,y+2,inst_id) && god_drop_char(nr,x-2,y+2,inst_id)) return 1;
        if (can_go(xs,ys,x-1,y+2,inst_id) && god_drop_char(nr,x-1,y+2,inst_id)) return 1;
        if (can_go(xs,ys,x+0,y+2,inst_id) && god_drop_char(nr,x+0,y+2,inst_id)) return 1;
        if (can_go(xs,ys,x+1,y+2,inst_id) && god_drop_char(nr,x+1,y+2,inst_id)) return 1;
        if (can_go(xs,ys,x-1,y-2,inst_id) && god_drop_char(nr,x-1,y-2,inst_id)) return 1;
        if (can_go(xs,ys,x+0,y-2,inst_id) && god_drop_char(nr,x+0,y-2,inst_id)) return 1;
        if (can_go(xs,ys,x+1,y-2,inst_id) && god_drop_char(nr,x+1,y-2,inst_id)) return 1;
...
int god_give_char(int in,int cn)
{
        ...
        it[in].y=0;
        it[in].carried=cn;
        it[in].instance_id=ch[cn].instance_id;
...
int god_transfer_char(int cn,int x,int y,int inst_id)
{
        int old_inst_id;
        ...
        old_inst_id = ch[cn].instance_id;
        if (god_drop_char_fuzzy_large(cn,x,y,x,y,inst_id)) return 1;
        if (god_drop_char_fuzzy_large(cn,x+3,y,x,y,inst_id)) return 1;
        if (god_drop_char_fuzzy_large(cn,x-3,y,x,y,inst_id)) return 1;
        if (god_drop_char_fuzzy_large(cn,x,y+3,x,y,inst_id)) return 1;
        if (god_drop_char_fuzzy_large(cn,x,y-3,x,y,inst_id)) return 1;
...
void god_createinst_base(int cn, char *name, char *fname, int wid, int hei)
{
        ...
        if (strlen(fname) < 3) {
                do_char_log(cn, 0, "Instance base filename should be longer than 2 characters.\n");
                return;
        }

        int ret=create_instance_base(name, fname, wid, hei);
        if (ret==-1) {
                do_char_log(cn, 0, "Failed to create new instance base, check server logs.\n");
                return;
        }
        do_char_log(cn, 1, "Created new instance base %s with ID %d.\n", name, ret);
}

void god_createinst_frombase(int cn, char *bname, int nochars)
{
        int ret=create_instance_frombase(bname, (short)nochars);
        if (ret==-1) {
                do_char_log(cn, 0, "Failed to create new instance from base %s, check server logs.\n", bname);
                return;
        }
        map_instances[ret].owner = ch[cn].player;
        do_char_log(cn, 1, "Created new instance from base %s, with ID %d.\n", bname, ret);
}
...
void god_goto_inst(int cn, int inst_id, int x, int y)
{
        ...
        if (inst_id == -1) {
                if (x < 0 || y < 0 || x >= MAPX || y >= MAPY) return;
        } else {
                if (!inst_isalive(inst_id)) return;
                if (x < 0 || y < 0 || x >= map_instances[inst_id].width || y >= map_instances[inst_id].height) return;
        }

        if (!(ch[cn].flags&CF_INVISIBLE)) fx_add_effect(12,0,ch[cn].x,ch[cn].y,0,old_inst);
        god_transfer_char(cn, x, y, inst_id);
        if (!(ch[cn].flags&CF_INVISIBLE)) fx_add_effect(12,0,x,y,0,inst_id);
}
...
void god_goto(int cn,int co,char *cx, char *cy)
{
        ...
        if (!*cy) { // GOTO to a character
                ...
                if (!IS_SANECHAR(who) || ((ch[who].flags&CF_GOD) && !(ch[cn].flags&CF_GOD))) {
                        ...
                        return;
                } else if (god_transfer_char(co,ch[who].x,ch[who].y,ch[who].instance_id)) {
                        chlog(cn, "IMP: transferred %s to %s (%d:%s).", ch[co].name,ch[who].name,ch[who].instance_id, get_area_m(ch[who].x,ch[who].y,0));
                        if (!(ch[co].flags&CF_INVISIBLE)) {
                                fx_add_effect(12,0,xo,yo,0,old_inst);
                                fx_add_effect(12,0,ch[co].x,ch[co].y,0,ch[co].instance_id);
                        }
                        return;
                }
                ...
                if (ch[co].instance_id == -1) {
                        if (x < 0 || x >= MAPX || y < 0 || y >= MAPY) {
                                do_char_log(cn,0,"Bad coordinates(%d %d)\n", x, y);
                                return;
                        }
                } else {
                        if (x < 0 || x >= map_instances[ch[co].instance_id].width || y < 0 || y >= map_instances[ch[co].instance_id].height) {
                                do_char_log(cn,0,"Bad coordinates(%d %d)\n", x, y);
                                return;
                        }
                }
                if (god_transfer_char(co,x,y,ch[co].instance_id)) {
...
void god_summon(int cn, char *spec1, char *spec2, char *spec3)
{
        ...
        if (!god_transfer_char(co, x, y, ch[cn].instance_id))  {
...
//	cc=pop_create_char(968,0);
    ...
    god_drop_char_fuzzy(cc,ch[cn].x,ch[cn].y,ch[cn].instance_id);   // to be replaced later by: _fuzzy(cc,ch[co].x,ch[co].y);
...
int god_thrall(int cn, char *spec1, char *spec2)
{
	...
	if ((in=ch[ct].citem) && (it[in].flags&IF_LABYDESTROY)) {
        ...
        if (!god_drop_char_fuzzy(ct, x, y, ch[cn].instance_id))  {
...
void god_setinsttemp(int cn, int tmp)
{
        if (ch[cn].instance_id == -1) {
                do_char_log(cn, 0, "This command is meant to be used during instance building.\n");
                return;
        }

        if (tmp == 0 || tmp == ch[cn].data[91]) {
                ch[cn].data[91] = 0;
                do_char_log(cn, 1, "No longer placing instanced character template.\n");
                return;
        }

        if (!IS_SANECTEMPLATE(tmp)) {
                do_char_log(cn, 0, "Template number out of bounds.\n");
                return;
        }

        if (ch_temp[tmp].used == USE_EMPTY) {
                do_char_log(cn, 0, "Unused template.\n");
                return;
        }

        if (ch_temp[tmp].flags&CF_PLAYER) {
                do_char_log(cn, 0, "Template reserved for players.\n");
                return;
        }

        ch[cn].data[91] = tmp;
        do_char_log(cn, 1, "Now placing instanced character template %d(%s).\n", tmp, ch_temp[tmp].name);
}

// Spawns a character from given template that remains motionless, doesn't react, and can be removed by looking at them (alt+click)
int god_createinstchar(int tmp, int x, int y, int inst_id)
{
        int ct,n,in;

        if (!(ct = god_create_char(tmp, 1))) return 0;

        // Make it reaction-less
        ch[ct].flags|=CF_INST_TEMP;

        ch[ct].flags&=~CF_RESPAWN;
	...
	if ((in=ch[ct].citem) && (it[in].flags&IF_LABYDESTROY)) {
        ...
        if (!god_drop_char_fuzzy(ct, x, y, inst_id))  {
                god_destroy_items(ct);
                ch[ct].used=USE_EMPTY;
                return 0;
        }

        strcpy(ch[ct].name,ch_temp[tmp].name);
        strcpy(ch[ct].reference,ch_temp[tmp].reference);
        strcpy(ch[ct].description,ch_temp[tmp].description);
...
void god_donate_item(int in,int place)
{
        ...
        if (!god_drop_item_fuzzy(in,x,y,-1)) it[in].used=USE_EMPTY;
}
...
void god_set_flag(int cn,int co,unsigned long long flag)
{
	...
	switch (flag) {
		...
		case    CF_INVISIBLE:   ptr="invisible"; fx_add_effect(12,0,ch[co].x,ch[co].y,0,ch[co].instance_id); break;
...
void god_set_purple(int cn,int co)
{
        ...
        if (!IS_PURPLE(co)) {
                ...
                ch[co].temple_x=HOME_MERCENARY_X;
        	ch[co].temple_y=HOME_MERCENARY_Y;
        } else {
...
void god_mail_pass(int cn,int co)
{
        ...
        sprintf(buf,"uuencode %s.moa </home/abaeterno/Desktop/av2sv/Server/.save/%s.moa | mail -s \"%s savefile\" %s",
                ch[co].name,
                ch[co].name,
                ch[co].name,
                "admin@astonia.com");
...
void god_exit_usurp(int cn)
{
        ...
        ch[co].player=nr;
        player[nr].usnr=co;
        god_transfer_char(co, HOME_MERCENARY_X, HOME_MERCENARY_Y, -1); // simulate recall
        do_afk(co, NULL);
...
void god_grolm(int cn)
{
        ...
        co=pop_create_char(386,1,ch[cn].instance_id);
        if (co) {
...
void god_gargoyle(int cn)
{
        ...
        co=pop_create_char(495,1,ch[cn].instance_id);
        if (co) {
