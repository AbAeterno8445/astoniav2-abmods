...
{
        int m,dir,x,y;
        int inst_id, inst_id_co, m_wid, m_hei;
        ...
        if (ch[co].tox<5 || ch[co].tox>m_wid-6 || ch[co].toy<5 || ch[co].toy>m_hei-6) return 0;

        if (!(IS_COMPANION(cn) && ch[cn].data[63]==co) && !do_char_can_see(cn,co)) return 0;

        m=ch[co].tox+ch[co].toy*m_wid;
        dir=ch[co].dir;

        switch(dir) {
                case DX_UP:             m=m+m_wid*2; break;
                case DX_DOWN:           m=m-m_wid*2; break;
                case DX_LEFT:           m=m+2; break;
                case DX_RIGHT:          m=m-2; break;
                case DX_LEFTUP:         m=m+2+m_wid*2; break;
                case DX_LEFTDOWN:       m=m+2-m_wid*2; break;
                case DX_RIGHTUP:        m=m-2+m_wid*2; break;
                case DX_RIGHTDOWN:      m=m-2-m_wid*2; break;
                default:                break;
        }

        if (inst_id == -1) {
                if (map[m].ch==cn || map[m+1].ch==cn || map[m-1].ch==cn || map[m+m_wid].ch==cn ||
                map[m-m_wid].ch==cn || map[m+1+m_wid].ch==cn || map[m+1-m_wid].ch==cn || map[m-1+m_wid].ch==cn ||
                map[m-1-m_wid].ch==cn) {
                ...
                if (map_instancedtiles[inst_id][m].ch==cn || map_instancedtiles[inst_id][m+1].ch==cn || map_instancedtiles[inst_id][m-1].ch==cn || map_instancedtiles[inst_id][m+m_wid].ch==cn ||
                map_instancedtiles[inst_id][m-m_wid].ch==cn || map_instancedtiles[inst_id][m+1+m_wid].ch==cn || map_instancedtiles[inst_id][m+1-m_wid].ch==cn || map_instancedtiles[inst_id][m-1+m_wid].ch==cn ||
                map_instancedtiles[inst_id][m-1-m_wid].ch==cn) {
        ...
        ch[cn].goto_x=m%m_wid;
        ch[cn].goto_y=m/m_wid;
...
void drv_useto(int cn,int x,int y)
{
        ...
        if (inst_id == -1) {
                ...
                if (x<0 || x>=map_instances[inst_id].width || y<0 || y>=map_instances[inst_id].height) x=y=0;
