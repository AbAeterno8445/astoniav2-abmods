...
int sv_areainst(unsigned char *buf)
{
	int mode, i;
	...
	if (mode == 0) {
		...
		// Load instance of area to be accessed
		area_insts[loaded_insts].time_left = *(unsigned short*)(buf+2);
		area_insts[loaded_insts].creation_time = *(unsigned short*)(buf+4);
		area_insts[loaded_insts].mobs_remaining = *(unsigned short*)(buf+6);
