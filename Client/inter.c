...
int mouse_shop(int x,int y,int mode)
{
	...
	if (x>mid_x && x<mid_x+330 && y>mid_y+1 && y<mid_y+292) {
		tx=(x-mid_x)/35;
		ty=(y-mid_y+1)/35;
...
int mouse_instmenu(int x,int y,int mode)
{
	int n;
	int mid_x,mid_y;
	...
	if (x>mid_x && x<mid_x+320 && y>mid_y+1 && y<mid_y+320) {
		if (x > mid_x + 16 && x < mid_x + 65) {
			if (mode==MS_LB_UP) {
